...
fn win32_public_poll_until_clipboard(mut app App, request ClipboardRequestId, attempts int) ![]WindowQueuedEvent {
	mut delivered := []WindowQueuedEvent{}
	for _ in 0 .. attempts {
		app.poll_events()!
		delivered << app.drain_window_queued_events()!
...
fn win32_public_poll_collect(mut app App, attempts int) ![]WindowQueuedEvent {
	mut delivered := []WindowQueuedEvent{}
	for _ in 0 .. attempts {
		app.poll_events()!
		delivered << app.drain_window_queued_events()!
...
fn test_win32_public_clipboard_cf_unicodetext_bmp_astral_roundtrip_red() {
	$if windows && gg_multiwindow ? {
		...
		assert C.v_multiwindow_test_win32_set_clipboard(hwnd, external.to_wide(),
			win32_public_utf16_units(external)) == 1
		...
		if read_request != ClipboardRequestId{} {
			...
			late_delivery := win32_public_poll_collect(mut app, 4)!
