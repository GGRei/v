...
pub fn initialize(params ManagerParams) &Manager {
	template_dir := if params.template_dir == '' {
...
fn (mut m Manager) compiled_template_for_path(source_path string) !&CompiledTemplate {
	...
	compiled := compile_template_from_file(source_path, m.template_extensions)!
...
fn compile_template_from_file(source_path string, template_extensions map[string]TemplateType) !&CompiledTemplate {
	template_type := template_type_from_path(source_path, template_extensions)!
	raw_content, dependencies := read_template_with_includes(source_path, 0)!
...
fn (m &Manager) resolve_template_path(template_path string) !string {
	...
	return os.real_path(source_path)
}
...
fn read_template_with_includes(source_path string, depth int) !(string, []TemplateDependency) {
	...
	for line in lines {
		expanded_line, line_dependencies := expand_include_directives(line, base_dir, depth)!
...
fn expand_include_directives(line string, base_dir string, depth int) !(string, []TemplateDependency) {
	...
	for {
		...
		include_path := resolve_include_path(base_dir, target)
		included, include_dependencies := read_template_with_includes(include_path, depth + 1)!
...
fn resolve_include_path(base_dir string, include_target string) string {
	mut target := include_target
	if os.file_ext(target) == '' {
		target += '.html'
	}
	if os.is_abs_path(target) {
		return os.real_path(target)
	}
	return os.real_path(os.join_path(base_dir, target))
}
