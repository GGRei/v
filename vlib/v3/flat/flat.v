...
pub struct DisabledConditionalFn {
pub:
	name        string
	module      string
	file        string
...
pub mut:
	nodes                    []Node
	children                 []NodeId
	disabled_conditional_fns []DisabledConditionalFn
	generic_template_decls   []bool
	user_code_start          int
}
...
pub fn (mut a FlatAst) mark_generic_template_decl(id NodeId) {
	if int(id) < 0 {
		return
	}
	for a.generic_template_decls.len <= int(id) {
		a.generic_template_decls << false
	}
	a.generic_template_decls[int(id)] = true
}

pub fn (a &FlatAst) is_generic_template_decl(id NodeId) bool {
	return int(id) >= 0 && int(id) < a.generic_template_decls.len
		&& a.generic_template_decls[int(id)]
}
