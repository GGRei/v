...
pub fn mark_used(a &flat.FlatAst, tc &types.TypeChecker) map[string]bool {
	...
	for node_idx, node in a.nodes {
		...
		if node.kind == .fn_decl {
			...
			fn_decls[qname] = info
			c_name := lowered_c_symbol_name(qname)
			if c_name != qname && c_name !in fn_decls {
				fn_decls[c_name] = info
			}
			if fn_decl_allows_short_lookup(cur_module) {
	...
	for qi < queue.len {
		...
		in_cg++
		if fn_info.canonical.len > 0 && fn_info.canonical !in used {
			used[fn_info.canonical] = true
		}
...
fn fn_decl_allows_short_lookup(module_name string) bool {
...
fn add_call(mut calls []CallRef, name string) {
	calls << CallRef{
		name: name
	}
}

fn add_exact_call(mut calls []CallRef, name string) {
	calls << CallRef{
		name:  name
		exact: true
	}
...
fn lowered_c_symbol_name(name string) string {
	...
	if n in markused_c_reserved_words {
		return 'v_${n}'
	}
	return n
}

const markused_c_reserved_words = ['auto', 'break', 'case', 'char', 'const', 'continue', 'copy',
	...
	'switch', 'typedef', 'union', 'unsigned', 'void', 'volatile', 'while', 'unix', 'linux']
...
fn (c &CallCollector) collect_calls(node &flat.Node, cur_module string, imports map[string]string, receiver_name string, receiver_struct string, mut calls []CallRef) {
	for i in 0 .. node.children_count {
		...
		c.collect_call_node(child_id, child, cur_module, imports, receiver_name, receiver_struct, mut
			calls)
	}
}

fn (c &CallCollector) collect_call_node(id flat.NodeId, node &flat.Node, cur_module string, imports map[string]string, receiver_name string, receiver_struct string, mut calls []CallRef) {
	if node.kind == .call && c.tc.disabled_conditional_call(id) {
		return
	}
	match node.kind {
		.ident {
			c.collect_fn_value_ident(id, node.value, cur_module, imports, mut calls)
		}
		.selector {
			c.collect_fn_value_selector(id, node, cur_module, imports, mut calls)
		}
		.call {
			mut has_resolved_call := false
			if resolved := c.tc.resolved_call_name(id) {
				add_call(mut calls, resolved)
				has_resolved_call = true
			}
			is_fn_value_call := c.tc.fn_value_call(id)
			if node.children_count > 0 {
				callee_id := c.a.child(node, 0)
				if int(callee_id) >= 0 {
					callee := c.a.nodes[int(callee_id)]
					if !has_resolved_call && !is_fn_value_call {
						if callee.kind == .ident && callee.value.len > 0 {
							add_call(mut calls, callee.value)
							qcallee := qualify_fn(cur_module, callee.value)
							if qcallee != callee.value {
								add_call(mut calls, qcallee)
							}
							...
							if callee.children_count > 0 {
								...
								if int(base_id) >= 0 {
									...
									if base.kind == .ident && base.value.len > 0 {
										...
										add_call(mut calls, mod_name + '.' + callee.value)
										add_call(mut calls, qualify_fn(cur_module, base.value +
											'.' + callee.value))
										if base.value.len > 0 && base.value[0] >= `A`
											...
											if named_type_name.len > 0 {
												add_call(mut calls, named_type_name + '.' +
													callee.value)
											}
										}
										...
										if int(inner_id) >= 0 {
											...
											if inner.kind == .ident && inner.value.len > 0 {
												...
												add_call(mut calls, mod_name + '.' + base.value +
													'.' + callee.value)
											}
										}
									}
									...
									if type_name.len > 0 {
										add_call(mut calls, type_name + '.' + callee.value)
									}
								}
							}
							add_call(mut calls, callee.value)
					...
					c.collect_selector_receiver_calls(callee, cur_module, imports, receiver_name,
						receiver_struct, mut calls)
				}
			}
			for ci in 1 .. node.children_count {
				arg_id := c.a.child(node, ci)
				if int(arg_id) >= 0 {
					if resolved := c.tc.resolved_fn_value_name(arg_id) {
						add_exact_call(mut calls, resolved)
					}
					arg := &c.a.nodes[int(arg_id)]
					c.collect_call_node(arg_id, arg, cur_module, imports, receiver_name,
						receiver_struct, mut calls)
				}
			}
			return
		...
		.prefix {
			if node.op == .amp && node.children_count > 0 {
				inner_id := c.a.child(node, 0)
				if int(inner_id) >= 0 {
					inner := c.a.nodes[int(inner_id)]
					if inner.kind == .struct_init {
						add_call(mut calls, 'memdup')
					}
				}
			}
		}
		.string_interp {
			add_call(mut calls, 'string_plus_many')
		}
		.infix {
			if node.op == .plus {
				add_call(mut calls, 'string__plus')
			}
			if node.children_count >= 2 {
				...
				if int(lhs_id) >= 0 {
					...
					if lhs_name.len > 0 {
						...
						if op_name.len > 0 {
							add_call(mut calls, lhs_name + '.' + op_name)
						}
					}
				}
			}
		...
		.struct_init {
			c.collect_struct_default_calls(node, imports, mut calls)
		}
		else {}
	}

	c.collect_calls(node, cur_module, imports, receiver_name, receiver_struct, mut calls)
}

fn (c &CallCollector) collect_selector_receiver_calls(callee &flat.Node, cur_module string, imports map[string]string, receiver_name string, receiver_struct string, mut calls []CallRef) {
	...
	base := &c.a.nodes[int(base_id)]
	c.collect_call_node(base_id, base, cur_module, imports, receiver_name, receiver_struct, mut
		calls)
}

fn (c &CallCollector) collect_fn_value_ident(id flat.NodeId, name string, cur_module string, imports map[string]string, mut calls []CallRef) {
...
fn (c &CallCollector) collect_fn_value_selector(id flat.NodeId, node &flat.Node, cur_module string, imports map[string]string, mut calls []CallRef) {
	if resolved := c.tc.resolved_fn_value_name(id) {
		add_exact_call(mut calls, resolved)
		return
	}
...
fn (c &CallCollector) node_is_fn_value(id flat.NodeId) bool {
	...
	if expr_type is types.FnType {
		return true
	}
	if expr_type is types.Alias {
		return expr_type.base_type is types.FnType
	}
	return false
}

fn (c &CallCollector) add_fn_value_candidates(name string, cur_module string, imports map[string]string, mut calls []CallRef) {
	if name.len == 0 {
		return
	}
	add_call(mut calls, name)
	qname := qualify_fn(cur_module, name)
	if qname != name {
		add_call(mut calls, qname)
	}
	if name.contains('.') {
		base := name.all_before_last('.')
		member := name.all_after_last('.')
		if base in imports {
			add_call(mut calls, imports[base] + '.' + member)
		}
	}
}

fn (c &CallCollector) add_const_alias_candidates(name string, cur_module string, imports map[string]string, mut calls []CallRef) bool {
	mut added := false
	for const_name in c.const_ref_candidates(name, cur_module, imports) {
		...
		match expr.kind {
			.ident {
				added = c.add_exact_fn_ref(expr.value, info.module, imports, mut calls) || added
			}
			.selector {
				if expr.children_count > 0 {
					...
					if base.kind == .ident && base.value.len > 0 && expr.value.len > 0 {
						added =
							c.add_exact_fn_ref('${base.value}.${expr.value}', info.module, imports, mut calls)
							|| added
						if base.value in imports {
							added =
								c.add_exact_fn_ref('${imports[base.value]}.${expr.value}', info.module, imports, mut calls)
								|| added
						}
					}
				}
...
fn (c &CallCollector) add_exact_fn_ref(name string, cur_module string, imports map[string]string, mut calls []CallRef) bool {
	for candidate in c.const_ref_candidates(name, cur_module, imports) {
		if candidate in c.fn_decls || candidate in c.tc.fn_ret_types {
			add_exact_call(mut calls, candidate)
			return true
		}
	}
	return false
...
fn (c &CallCollector) collect_struct_default_calls(init &flat.Node, imports map[string]string, mut calls []CallRef) {
