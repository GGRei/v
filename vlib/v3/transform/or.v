...
fn (t &Transformer) or_value_type(base_name string, expected_type string) string {
	expected := t.optional_base_type(expected_type)
	if expected.len > 0 && expected !in ['?', '!', 'void', 'Optional'] {
		return expected
	}
	return base_name
}
	...
	if !isnil(t.tc) {
		if typ := t.tc.expr_type(expr_id) {
			if typ is types.OptionType {
				...
				return source_name, t.or_value_type(base_name, fallback_type)
			}
...
fn (mut t Transformer) lower_or_expr_to_temp(id flat.NodeId, node flat.Node) flat.NodeId {
	...
	source_value_type := t.optional_base_type(expr_type)
	value_expr := t.make_selector(t.make_ident(opt_tmp), 'value', source_value_type)
	then_value := t.wrap_sum_expr_for_type(value_expr, value_type)
	...
	for i in 0 .. body.children_count {
		...
		if is_last && child.kind == .expr_stmt && child.children_count > 0 {
			...
			if inner.kind == .call && t.is_noreturn_call(inner) {
				...
				value := t.wrap_sum_expr_for_type(inner_id, target_type)
				t.drain_pending(mut result)
