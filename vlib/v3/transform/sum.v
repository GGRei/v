...
fn (mut t Transformer) wrap_sum_return_expr(expr_id flat.NodeId) flat.NodeId {
	return t.wrap_sum_expr_for_type(expr_id, t.cur_fn_ret_type)
}

fn (mut t Transformer) wrap_sum_expr_for_type(expr_id flat.NodeId, target_type string) flat.NodeId {
	resolved_sum := t.resolve_sum_name(t.trim_pointer_type(target_type))
	mut variants := []string{}
	if vals := t.sum_types[resolved_sum] {
		variants = vals.clone()
	} else if !isnil(t.tc) {
		if vals := t.tc.sum_types[resolved_sum] {
			variants = vals.clone()
		}
	}
	if resolved_sum.len == 0 || variants.len == 0 {
		return t.transform_expr(expr_id)
	}
