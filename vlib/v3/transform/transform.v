...
fn (mut t Transformer) transform_decl_assign_stmt(id flat.NodeId, node flat.Node) []flat.NodeId {
	...
	if node.children_count == 2 {
		...
		if lhs.kind == .ident && lhs.value.len > 0 {
			...
			if node.typ.len == 0 {
				rhs_id := t.a.child(&node, 1)
				rhs := t.a.nodes[int(rhs_id)]
				if rhs.kind == .array_literal && rhs.value != 'fixed' && is_fixed_array_type(typ) {
	...
	for i in 0 .. node.children_count {
		...
		if i == 0 || (node.children_count > 2 && i > 1) {
			...
			child := t.a.nodes[int(child_id)]
			if child.kind == .or_expr && inferred_typ.len > 0 {
				t.a.nodes[int(child_id)].typ = inferred_typ
			}
			new_children << t.transform_expr(child_id)
		}
	}
...
fn (t &Transformer) infer_decl_type(node &flat.Node) string {
	if node.typ.len > 0 {
		if node.children_count >= 2 && !isnil(t.tc) {
			rhs_id := t.a.child(node, 1)
			rhs := t.a.nodes[int(rhs_id)]
			if rhs.kind == .array_literal && rhs.value == 'fixed' {
				if rhs_type := t.tc.expr_type(rhs_id) {
					type_name := rhs_type.name()
					if is_fixed_array_type(type_name) {
						return type_name
					}
				}
			}
	...
	match node.kind {
		...
		.or_expr {
			if node.children_count > 0 {
				...
				inner_type := if inner.kind == .index {
					t.resolve_index_elem_type(inner)
				} else {
					t.node_type(inner_id)
				}
