...
fn (t &Transformer) resolve_index_elem_type(node flat.Node) string {
	...
	if base_type.starts_with('&') {
		...
		if inner_type.starts_with('[]') {
			return t.normalize_type_alias(inner_type[2..])
		}
		if is_fixed_array_type(inner_type) {
			return t.normalize_type_alias(fixed_array_elem_type(inner_type))
		}
		if inner_type.starts_with('map[') {
			bracket_end := inner_type.index(']') or { return '' }
			if bracket_end + 1 < inner_type.len {
				return t.normalize_type_alias(inner_type[bracket_end + 1..])
			}
			return ''
		}
		return t.normalize_type_alias(inner_type)
	}
	if node.value == 'range' {
		if base_type == 'string' {
			return 'string'
