...
fn (t &Transformer) lookup_struct_info(name string) ?StructInfo {
	if name.contains('.') {
		...
		if info := t.lookup_struct_info_alias(name) {
			return info
		}
		return none
	}
	if t.cur_module.len > 0 && t.cur_module != 'main' && t.cur_module != 'builtin' {
		...
		if info := t.lookup_struct_info_alias(qname) {
			return info
		}
	}
	if name in t.structs {
...
fn (t &Transformer) lookup_struct_info_alias(name string) ?StructInfo {
	if isnil(t.tc) {
		return none
	}
	mut current := name
	...
	for _ in 0 .. 8 {
		...
		if info := t.lookup_struct_info_alias_target(current, target) {
			return info
		}
		next := t.alias_target_name(current, target)
		if next.len == 0 || next == current {
			return none
		}
		current = next
	}
...
fn (t &Transformer) lookup_struct_info_alias_target(alias_name string, target string) ?StructInfo {
	for candidate in t.alias_target_candidates(alias_name, target) {
		if candidate in t.structs {
			return t.structs[candidate]
		}
	}
	return none
}

fn (t &Transformer) alias_target_name(alias_name string, target string) string {
	for candidate in t.alias_target_candidates(alias_name, target) {
...
fn (t &Transformer) alias_target_candidates(alias_name string, target string) []string {
	mut candidates := []string{}
	if alias_name.contains('.') {
		alias_mod := alias_name.all_before_last('.')
		if target.starts_with('C.') {
			candidates << '${alias_mod}.${target}'
