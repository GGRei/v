...
fn v3_native_source_compile_flags(plan_flags []string, source_root string, language string) []string {
	...
	for i < plan_flags.len {
		...
		is_cpp_language := language in ['c++', 'objective-c++']
		if (clean.starts_with('-x') && clean.len > 2)
			|| v3_cpp_compile_flag_is_input_output_or_link_only(clean)
			|| (!clean.starts_with('-') && (c_flag_is_c_source_file(clean)
			|| os.file_ext(clean) == '.C'))
			|| (is_cpp_language && v3_is_c_language_standard_token(clean))
			|| (!is_cpp_language && v3_is_cpp_language_standard_token(clean)) {
			i++
			continue
		}
...
fn v3_cpp_source_compile_flags(plan_flags []string, source_root string) []string {
	return v3_native_source_compile_flags(plan_flags, source_root, 'c++')
}

fn v3_c_source_compile_flags(plan_flags []string, source_root string, language string, c_standard string) []string {
	mut flags := v3_native_source_compile_flags(plan_flags, source_root, language)
	if c_standard.len > 0 {
		flags = flags.filter(it != c_standard)
		flags << c_standard
	}
	return flags
}

fn v3_native_source_compile_args(before_inputs []string, after_inputs []string, source_root string, language string, c_standard string, input string, output string) []string {
	mut args := if language in ['c++', 'objective-c++'] {
		v3_native_source_compile_flags(before_inputs, source_root, language)
	} else {
		v3_c_source_compile_flags(before_inputs, source_root, language, c_standard)
	}
	args << ['-x', language, '-c', input, '-o', output]
	args << v3_native_source_compile_flags(after_inputs, source_root, language)
	return args
}
...
fn prepare_v3_cpp_linker_source_objects(c_compiler string, cpp_compiler string, args []string, before_inputs []string, after_inputs []string, c_standard string, source_root string, cc_dir string, show_cc bool, mut temporary_files []string) ![]string {
	mut result := []string{cap: args.len + 8}
	mut explicit_language := ''
	mut c_object_index := 0
	mut cpp_object_index := 0
	mut i := 0
	for i < args.len {
		arg := args[i]
		clean := arg.trim_space()
		...
		if language !in ['c', 'objective-c', 'c++', 'objective-c++'] && !(language == ''
			&& os.file_ext(path) == '.c') {
			result << arg
			i++
			continue
		}
		source_language := if language == '' { 'c' } else { language }
		is_cpp_source := source_language in ['c++', 'objective-c++']
		object_path := os.join_path_single(cc_dir, if is_cpp_source {
			'v3_cpp_source_${cpp_object_index}.o'
		} else {
			'v3_c_source_${c_object_index}.o'
		})
		if is_cpp_source {
			cpp_object_index++
		} else {
		...
		compiler := if is_cpp_source { cpp_compiler } else { c_compiler }
		compile_args := v3_native_source_compile_args(before_inputs, after_inputs, source_root,
			source_language, c_standard, v3_cpp_link_input_path(path, source_root, cc_dir),
			os.base(object_path))
		if show_cc {
			println('  > ${cmdexec.display(compiler, compile_args)}')
		}
		compile_result := cmdexec.run_in(compiler, compile_args, cc_dir)
		if compile_result.exit_code != 0 || !os.is_file(object_path) {
			return error('failed to compile ${source_language} source `${path}` with `${compiler}`:\n${compile_result.output}')
		}
		result << object_path
		i++
	}
...
struct V3CacheCCompiler {
	family     string
	executable string
}

fn v3_cache_c_compiler(family string, compiler string) V3CacheCCompiler {
	return V3CacheCCompiler{
		family:     family
		executable: if os.is_abs_path(compiler) {
			compiler
		} else {
...
fn prepare_v3_cache_external_inputs(mut state V3ModuleCacheState, a &flat.FlatAst, prefs &pref.Preferences, user_files []string, user_c_flags []string, compiler V3CacheCCompiler) bool {
	...
	compiler_macros, compiler_macro_environment_complete := cache_c_compiler_predefined_macros(user_c_flags,
		compiler.executable, prefs.target, native_inputs_language)
	...
	mut native_source_modules, can_scope_static_inputs := cache_external_input_owner_modules(state,
		a, unscoped_inputs, static_storage_inputs, user_files, user_c_flags, compiler.executable,
		prefs.target)
...
fn prepare_v3_cache_external_inputs_scoped(mut state V3ModuleCacheState, a &flat.FlatAst, prefs &pref.Preferences, user_files []string, user_c_flags []string, scope_enabled bool, compiler V3CacheCCompiler) bool {
	if !scope_enabled {
...
fn prepare_v3_checker_native_inputs(mut state V3ModuleCacheState, a &flat.FlatAst, prefs &pref.Preferences, user_files []string, user_c_flags []string, compiler V3CacheCCompiler) {
	mut cache_input_modules := map[string]bool{}
	for module_name in state.module_sources.keys() {
		cache_input_modules[module_name] = true
	}
...
fn prepare_v3_checker_native_inputs_scoped(mut state V3ModuleCacheState, a &flat.FlatAst, prefs &pref.Preferences, user_files []string, user_c_flags []string, scope_enabled bool, compiler V3CacheCCompiler) {
	if !scope_enabled {
		prepare_v3_checker_native_inputs(mut state, a, prefs, user_files, user_c_flags, compiler)
		return
	}
	scope := prealloc_scope_begin_for_v3()
	prepare_v3_checker_native_inputs(mut state, a, prefs, user_files, user_c_flags, compiler)
...
fn prepare_v3_checker_native_inputs_thread(args &PrepareV3CheckerNativeInputsArgs) {
	...
	prepare_v3_checker_native_inputs_scoped(mut state, args.a, args.prefs, args.user_files,
		args.user_c_flags, args.scope_enabled, args.compiler)
	args.done <- true
}
...
fn prepare_v3_cache_native_type_declarations(mut state V3ModuleCacheState, c_flags []string, compiler V3CacheCCompiler, target pref.Target) bool {
	...
	for raw_module_name, roots in state.module_native_roots {
		...
		mut declaration_macros := cache_local_c_compiler_macros(c_flags, compiler.family, target)
		...
		if !declarations_complete_for_module {
			...
			for root in roots {
				...
				preprocessed := cache_preprocessed_native_input(real_root, state.native_root_contexts[real_root] or {
					[]string{}
				}, c_flags, compiler.executable, target) or { continue }
		...
		mut include_macros := cache_local_c_compiler_macros(c_flags, compiler.family, target)
		...
		for root in roots {
			...
			if roots_with_function_declarations[real_root] && !c_flag_is_c_source_file(real_root) {
				...
				implementation_macros := cache_native_implementation_context_macros(real_root,
					context, allowed_paths, c_flags, compiler.family, target)
				...
				if cache_native_public_include_replays_external_definition(real_root, context,
					implementation_macros, allowed_paths, c_flags, compiler, target)
				{
					if os.getenv('V3_CACHE_TRACE') != '' {
...
// to verify is unsafe.
fn cache_native_public_include_replays_external_definition(path string, context []string, implementation_macros map[string]bool, allowed_paths map[string]bool, c_flags []string, compiler V3CacheCCompiler, target pref.Target) bool {
	...
	mut replay_macros := cache_local_c_compiler_macros(c_flags, compiler.family, target)
	...
	if preprocessed := cache_preprocessed_native_input(path, replay_context, c_flags,
...
fn restore_v3_cache_external_inputs(mut state V3ModuleCacheState, user_files []string, user_c_flags []string, compiler V3CacheCCompiler, target pref.Target, incremental_declaration_signature string, native_link_requirements types.NativeLinkRequirements) bool {
	...
	if !prepare_v3_cache_native_type_declarations(mut state, user_c_flags, compiler, target) {
		return false
	}
...
fn capture_v3_fastc_c14_source(source string) {
	$if linux {
		phase := os.getenv('V3_FASTC_C14_PHASE')
		name := match phase {
			'repeat-1-of-2' { 'gen1.c' }
			'selfhost-build' { 'selfhost.c' }
			else { return }
		}
		dir := os.getenv('V3_FASTC_C14_DIR')
		if dir == '' || !os.is_abs_path(dir) || !os.is_dir(dir) || os.is_link(dir)
			|| os.real_path(dir) != dir || source.len < 1 || source.len > 8 * 1024 * 1024 {
			return
		}
		...
		if os.exists(path) || os.is_link(path) || os.exists(tmp) || os.is_link(tmp)
			|| os.exists(meta_path) || os.is_link(meta_path) || os.exists(meta_tmp)
			|| os.is_link(meta_tmp) {
			return
		}
		...
		meta := 'phase=${phase}\npid=${os.getpid()}\n'
		os.write_file(meta_tmp, meta) or {
			os.rm(tmp) or {}
			return
		}
		if meta.len > 256 || !os.is_file(meta_tmp) || os.is_link(meta_tmp) {
			os.rm(tmp) or {}
			os.rm(meta_tmp) or {}
			return
		}
...
fn compile_v3_fastc_source(source string, bin_file string, prefs &pref.Preferences, environment_c_flags []string, source_c_flags []string, user_c_flags []string, environment_ld_flags []string, is_debug bool, uses_threads bool) V3FastCCompileResult {
	...
	$if linux {
		if os.getenv('V3_FASTC_C14_DIR') != '' {
			capture_v3_fastc_c14_source(source)
		}
	}
...
pub fn run(args []string) {
	...
	cache_c_compiler := v3_cache_c_compiler(prefs.ccompiler, c_compiler)
	...
	if is_direct_vsh && should_run && !explicit_output {
		...
		if os.getenv(v3_internal_restart_env) == '1' && carried_identity.len > 0 {
			...
			_ = prepare_v3_cache_external_inputs_scoped(mut cache_state, a, prefs, user_files,
				crun_c_flags, scope_prealloc_stages, cache_c_compiler)
	...
	if backend == 'c' && program_cache_enabled && !cache_state.force_source
		...
		mut external_inputs_ready := restore_v3_cache_external_inputs(mut cache_state, user_files,
			cache_c_flags, cache_c_compiler, prefs.target, '', native_link_requirements)
		if !external_inputs_ready && incremental_cache_enabled {
			...
			external_inputs_ready = restore_v3_cache_external_inputs(mut cache_state, user_files,
				cache_c_flags, cache_c_compiler, prefs.target,
				incremental_snapshot.declaration_signature, native_link_requirements)
		}
		if !external_inputs_ready
			&& !prepare_v3_cache_external_inputs_scoped(mut cache_state, a, prefs, user_files, cache_c_flags, scope_prealloc_stages, cache_c_compiler) {
	...
	if native_inputs_overlap {
		...
		if cache_state.manager.enabled {
			_ = prepare_v3_cache_external_inputs_scoped(mut cache_state, a, prefs, user_files,
				cache_c_flags, scope_prealloc_stages, cache_c_compiler)
		} else {
	...
	if backend == 'arm64' {
		...
		if !c_only || (dump_c_flags.len > 0 && generate_c_project.len == 0) {
			resolved_c_flags = prepare_c_flags_for_link(generated_c_flags, environment_c_flags,
				prefs.c99, pic_flag, target_args, prefs.target, c_compiler, cc_dir, link_with_cpp, mut
		...
		if is_prod || !tried_tcc || result.exit_code != 0 {
			...
			if link_with_cpp {
				...
				cpp_args = prepare_v3_cpp_linker_source_objects(c_compiler, active_linker_driver,
					cpp_args, c_flag_plan.before_inputs, c_flag_plan.after_inputs, c_standard,
					source_root, cc_dir, !silent || show_cc, mut cpp_linker_temporary_files) or {
					eprintln(err.msg())
			...
			if result.exit_code != 0 {
				...
				if request_macos_v3_c_error_fallback(macos_v3_fallback_file, macos_v3_c_error_dir,
					active_linker_driver, result.output, os.join_path_single(cc_dir,
					fallback_source), fallback_report_sources)
				{
					cleanup_c_build_dir(cc_dir)
					exit(1)
				}
