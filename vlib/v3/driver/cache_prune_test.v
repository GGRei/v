...
fn v3_cache_c_compiler_from_scoped_cwd(root string, family string, compiler string) V3CacheCCompiler {
	old_wd := os.getwd()
	defer {
		os.chdir(old_wd) or { panic(err) }
	}
	os.chdir(root) or { panic(err) }
	return v3_cache_c_compiler(family, compiler)
}
...
') or {
	...
	missing_absolute_path := os.join_path(root, 'missing absolute compiler${recorder_suffix}')
	assert !os.exists(missing_absolute_path)
	assert v3_cache_c_compiler('tinyc', missing_absolute_path).executable == missing_absolute_path
	...
	os.write_file(recorder_log, '') or { panic(err) }
	relative_recorder := '.${os.path_separator}${recorder_name}'
	assert !os.is_abs_path(relative_recorder)
	relative := v3_cache_c_compiler_from_scoped_cwd(root, 'tinyc', relative_recorder)
	assert relative.family == 'tinyc'
	assert relative.executable == recorder
