...
fn test_v3_native_source_compile_flags_keep_only_the_selected_language_standard() {
	...
	c99_index := c_args.index('-std=c99') or { -1 }
	gnu11_index := c_args.index('-std=gnu11') or { -1 }
	c_input_index := c_args.index('src.c') or { -1 }
	c_output_index := c_args.index('src.o') or { -1 }
	c17_index := c_args.index('-std=c17') or { -1 }
	...
	cpp17_index := cpp_args.index('-std=c++17') or { -1 }
	cpp_input_index := cpp_args.index('src.cpp') or { -1 }
	cpp_output_index := cpp_args.index('src.o') or { -1 }
	cpp23_index := cpp_args.index('-std=c++23') or { -1 }
