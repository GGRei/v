...
fn (mut p Parser) top_level_items() []flat.NodeId {
...
pub fn synthesize_top_level_main(mut a flat.FlatAst, user_files []string) []string {
	mut user_file_set := map[string]bool{}
	for file in user_files {
		user_file_set[file] = true
	}
	if user_file_set.len == 0 {
		return []
	}

	mut errors := []string{}
	mut explicit_main := false
	mut stmt_ids := []flat.NodeId{}
	mut stmt_file_nodes := []flat.NodeId{}
	mut seen_stmt_file_node := map[int]bool{}
	mut main_file := ''
	mut main_pos := token.Pos{}

	for i, node in a.nodes {
		if node.kind != .file || node.children_count == 0 || !user_file_set[node.value] {
			continue
		}
		mut module_name := ''
		mut file_stmt_ids := []flat.NodeId{}
		for child_index in 0 .. node.children_count {
			...
			if child.kind == .fn_decl && child.value == 'main'
				&& (module_name.len == 0 || module_name == 'main') {
				explicit_main = true
			}
			if is_top_level_executable_stmt_kind(child.kind) {
				file_stmt_ids << child_id
				if main_file.len == 0 {
					main_file = node.value
					main_pos = child.pos
				}
			}
		}
		if file_stmt_ids.len == 0 {
			continue
		}
		if module_name.len > 0 && module_name != 'main' {
			errors << 'top-level statements are only supported in module `main`: ${node.value}'
			continue
		}
		for id in file_stmt_ids {
			stmt_ids << id
		}
		if !seen_stmt_file_node[i] {
			stmt_file_nodes << flat.NodeId(i)
			seen_stmt_file_node[i] = true
		}
	}
	...
	if explicit_main {
		errors << 'top-level statements cannot be mixed with an explicit `fn main()`'
		return errors
	}
	if errors.len > 0 {
		return errors
	...
	mut removed_stmt_ids := map[int]bool{}
	for id in stmt_ids {
		removed_stmt_ids[int(id)] = true
	}
	mut added_main_to_file := false
	for file_node_id in stmt_file_nodes {
		node := a.nodes[int(file_node_id)]
		mut children := []flat.NodeId{}
		for child_index in 0 .. node.children_count {
			child_id := a.child(&node, child_index)
			if removed_stmt_ids[int(child_id)] {
				continue
			}
			children << child_id
		}
		if !added_main_to_file {
			children << main_id
			added_main_to_file = true
		}
		child_start := a.begin_children()
		for child_id in children {
			a.add_child(child_id)
		}
		a.nodes[int(file_node_id)] = flat.Node{
			kind:           .file
			value:          node.value
			typ:            node.typ
			pos:            node.pos
...
fn is_top_level_executable_stmt_kind(kind flat.NodeKind) bool {
...
fn (mut p Parser) top_level_stmt() flat.NodeId {
	...
	match p.tok {
		...
		.attribute {
			p.skip_decl_attrs()
			if p.skip_next_decl {
				cond := p.skip_next_decl_cond
				p.skip_next_decl = false
				p.skip_next_decl_cond = ''
				p.clear_pending_decl_attrs()
				p.skip_disabled_conditional_decl(cond)
				p.clear_pending_decl_attrs()
...
fn (mut p Parser) clear_pending_decl_attrs() {
	p.pending_params = false
	p.pending_flag = false
}

fn (mut p Parser) clear_unusable_decl_attrs() {
	if p.pending_params
		&& p.tok !in [.attribute, .lsbr, .semicolon, .key_pub, .key_struct, .key_union] {
		p.pending_params = false
	}
	if p.pending_flag && p.tok !in [.attribute, .lsbr, .semicolon, .key_pub, .key_enum] {
		p.pending_flag = false
	}
}
...
fn (mut p Parser) fn_decl() flat.NodeId {
	...
	if p.tok == .lpar {
		...
		if is_mut && !receiver_type.starts_with('&') {
...
fn (mut p Parser) fn_decl_body(name string, receiver_name string, receiver_type string, is_method bool, _ bool) flat.NodeId {
	mut generic_marker := ''
	if p.tok == .lsbr {
		generic_marker = p.parse_generic_param_marker()
	}
	...
	if p.tok != .lcbr {
		...
		if generic_marker.len > 0 {
			p.a.mark_generic_template_decl(id)
		}
		return id
	}
...
fn (mut p Parser) parse_param_group_types() []string {
	mut types := []string{}
	mut names := []string{}
	mut is_mut := false
	if p.tok == .key_mut {
...
fn (mut p Parser) struct_decl() flat.NodeId {
	...
	for p.tok != .rcbr && p.tok != .eof {
		...
		if p.tok == .name || p.tok.is_keyword() {
			...
			if p.tok == .dot {
				mut embedded_type := field_name
				for p.tok == .dot {
					p.next()
					embedded_type += '.' + p.expect_name_or_keyword()
				}
				ids << p.a.add_node(flat.Node{
					kind:  .field_decl
					value: embedded_type.all_after_last('.')
					typ:   embedded_type
				})
				if p.tok == .semicolon {
					p.next()
...
fn (mut p Parser) skip_decl_attrs() {
	p.skip_attrs_inner(true)
}

fn (mut p Parser) skip_attrs() {
	p.skip_attrs_inner(false)
}

fn (mut p Parser) skip_attrs_inner(collect_decl_attrs bool) {
	if p.tok == .attribute {
		p.next()
		if collect_decl_attrs && p.tok == .name && p.lit == 'flag' {
			p.pending_flag = true
		} else if collect_decl_attrs && p.tok == .name && p.lit == 'params' {
		...
		if p.tok == .key_if {
			...
			cond_str := cond.str()
			if !eval_comptime_cond(p.prefs, cond_str) {
				p.skip_next_decl = true
				p.skip_next_decl_cond = cond_str
			}
		}
		for p.tok != .rsbr && p.tok != .eof {
			p.next()
...
fn (mut p Parser) skip_disabled_conditional_decl(cond string) {
	for p.tok == .semicolon {
		p.next()
	}
	...
	if p.tok == .key_fn {
		p.skip_disabled_conditional_fn_decl(cond)
		return
	}
	p.skip_top_level_stmt()
}

fn (mut p Parser) skip_disabled_conditional_fn_decl(cond string) {
	p.check(.key_fn)
	mut name := ''
	mut receiver_type := ''
	mut is_method := false
	...
	if p.tok == .lpar {
		...
		if p.tok != .name && p.tok != .eof && p.tok.is_overloadable() {
			op_name := overload_token_name(p.tok)
			p.next()
			clean_type := receiver_type.trim_left('&')
			name = '${clean_type}.${op_name}'
			p.skip_disabled_conditional_fn_body(name, receiver_type, is_method, cond)
			return
		}
	}
...
fn (mut p Parser) skip_disabled_conditional_fn_body(name string, receiver_type string, is_method bool,
	cond string) {
	// generic params -- skip
	if p.tok == .lsbr {
		p.skip_brackets()
	...
	for p.tok != .rpar && p.tok != .eof {
		start_offset := p.s.offset
		param_types << p.parse_param_group_types()
		if p.s.offset == start_offset && p.tok != .rpar && p.tok != .eof {
			p.next()
		}
	}
	...
	if name.len > 0 {
		p.a.disabled_conditional_fns << flat.DisabledConditionalFn{
			name:        name
			module:      p.cur_module
			file:        p.cur_file
			cond:        cond
...
fn (mut p Parser) parse_top_level_comptime_if() []flat.NodeId {
	p.next() // skip $
	if p.tok != .key_if {
		// $for or other comptime forms are not top-level declarations here.
		if p.tok == .key_for {
			p.next()
			for p.tok != .lcbr && p.tok != .eof {
				p.next()
	...
	taken := eval_comptime_cond(p.prefs, cond)
	if taken {
		ids := p.parse_top_level_block_items()
		p.skip_comptime_else()
		return ids
	}
	p.skip_block()
...
fn (mut p Parser) parse_top_level_block_items() []flat.NodeId {
	p.check(.lcbr)
	mut ids := []flat.NodeId{}
	for p.tok != .rcbr && p.tok != .eof {
		if p.tok == .semicolon {
...
fn (mut p Parser) parse_top_level_comptime_else() []flat.NodeId {
	...
	return p.parse_top_level_block_items()
}
...
fn (mut p Parser) parse_generic_param_marker() string {
	if p.tok != .lsbr {
		return ''
	}
	mut params := []string{}
	...
	for depth > 0 && p.tok != .eof {
		...
		if depth == 1 && (p.tok == .name || p.tok.is_keyword()) {
			params << p.lit
		}
		p.next()
	}
...
fn (mut p Parser) if_stmt() flat.NodeId {
	p.next() // skip 'if'
	cond := p.expr_until_block(.lowest)
	...
	if p.tok == .comma || p.tok == .decl_assign {
		...
		if p.tok == .decl_assign {
			...
			if p.tok == .decl_assign {
				p.next()
				p.expr_until_block(.lowest) // consume RHS
			}
		}
	}
...
fn (mut p Parser) for_stmt() flat.NodeId {
	...
	first_expr := p.expr_until_block(.lowest)
	...
	if p.tok == .semicolon {
		...
		if pk == .semicolon || pk == .lcbr {
			...
			post := if p.tok != .lcbr && p.tok != .eof {
				p.assign_or_expr_inline_until_block()
			} else {
...
fn (mut p Parser) for_c_style(lhs_expr flat.NodeId) flat.NodeId {
	...
	post := p.assign_or_expr_inline_until_block()
...
fn (mut p Parser) for_in(first_expr flat.NodeId) flat.NodeId {
	...
	p.check(.key_in)
	container := p.expr_until_block(.lowest)

	// optional range: `for i in 0 .. n`
	mut range_end := flat.empty_node
	if p.tok == .dotdot {
		p.next()
		range_end = p.expr_until_block(.lowest)
	}
...
fn (mut p Parser) match_stmt() flat.NodeId {
	p.next() // skip 'match'
	match_expr := p.expr_until_block(.lowest)
	p.check(.lcbr)
...
fn (mut p Parser) assign_or_expr_inline() flat.NodeId {
	return p.assign_or_expr_inline_mode(false)
}

fn (mut p Parser) assign_or_expr_inline_until_block() flat.NodeId {
	return p.assign_or_expr_inline_mode(true)
}

fn (mut p Parser) assign_or_expr_inline_mode(stop_before_block bool) flat.NodeId {
	lhs := p.expr_mode(.lowest, stop_before_block)

	if p.tok.is_assignment() {
		op := p.tok
		p.next()
		rhs := p.expr_mode(.lowest, stop_before_block)
		lhs_node := p.a.nodes[int(lhs)]
		kind := if lhs_node.kind == .selector {
			flat.NodeKind.selector_assign
		} else if lhs_node.kind == .index {
...
fn (mut p Parser) expr(min_bp token.BindingPower) flat.NodeId {
	return p.expr_mode(min_bp, false)
}

fn (mut p Parser) expr_until_block(min_bp token.BindingPower) flat.NodeId {
	return p.expr_mode(min_bp, true)
}

fn (mut p Parser) expr_mode(min_bp token.BindingPower, stop_before_block bool) flat.NodeId {
	lhs := p.prefix_expr()
	return p.expr_with_lhs_mode(lhs, min_bp, stop_before_block)
}

fn (mut p Parser) expr_with_lhs(first flat.NodeId, min_bp token.BindingPower) flat.NodeId {
	return p.expr_with_lhs_mode(first, min_bp, false)
}

fn (mut p Parser) expr_with_lhs_mode(first flat.NodeId, min_bp token.BindingPower, stop_before_block bool) flat.NodeId {
	mut lhs := first
	for {
		...
		if p.tok == .lcbr {
			...
			if lhs_node.kind == .selector && lhs_node.value.len > 0 {
				...
				if base.kind == .ident
					...
					if !stop_before_block || p.lcbr_starts_struct_literal_before_block() {
		...
		if p.tok == .key_in || p.tok == .not_in {
			...
			p.next()
			mut rhs := p.expr_mode(token.BindingPower.bit_or, stop_before_block)
			if p.tok == .dotdot {
				p.next()
				range_rhs := p.expr_mode(.lowest, stop_before_block)
		...
		op := p.tok
		p.next()
		rhs := p.expr_mode(op.right_binding_power(), stop_before_block)
		istart := p.add_children2(lhs, rhs)
		lhs = p.a.add_node(flat.Node{
			kind:           .infix
			op:             token_to_op(op)
...
fn (p &Parser) lcbr_starts_struct_literal_before_block() bool {
	return source_lcbr_starts_struct_literal_before_block(p.s.src, p.s.pos)
}

fn source_lcbr_starts_struct_literal_before_block(src string, lcbr_pos int) bool {
	if lcbr_pos < 0 || lcbr_pos >= src.len || src[lcbr_pos] != `{` {
		return false
	}
	cbr_pos := source_matching_cbr(src, lcbr_pos)
	if cbr_pos < 0 {
		return false
	}
	after := source_skip_space_and_comments(src, cbr_pos + 1)
	return source_after_complete_struct_literal_allows_control_expr(src, after)
}

fn source_matching_cbr(src string, lcbr_pos int) int {
	mut depth := 0
	mut i := lcbr_pos
	for i < src.len {
		c := src[i]
...
fn source_skip_quoted(src string, quote_pos int) int {
	quote := src[quote_pos]
	mut i := quote_pos + 1
	for i < src.len {
		c := src[i]
		if c == `\\` && quote != `\`` && i + 1 < src.len {
			i += 2
			continue
		}
		if c == quote {
...
fn source_after_complete_struct_literal_allows_control_expr(src string, pos int) bool {
	if pos >= src.len {
		return false
	}
	c := src[pos]
	...
	if source_is_ident_start(c) {
		end := source_scan_ident(src, pos)
		word := src[pos..end]
		return word == 'is' || word == 'in' || word == 'or' || word == 'as'
	}
	return false
}

fn source_skip_space_and_comments(src string, start int) int {
	mut i := start
	for i < src.len {
		c := src[i]
		if c == ` ` || c == `\t` || c == `\n` || c == `\r` {
...
fn source_is_ident_start(c u8) bool {
	return (c >= `a` && c <= `z`) || (c >= `A` && c <= `Z`) || c == `_`
}

fn source_is_ident_char(c u8) bool {
	return source_is_ident_start(c) || (c >= `0` && c <= `9`)
}

fn source_scan_ident(src string, start int) int {
	mut i := start
	for i < src.len && source_is_ident_char(src[i]) {
		i++
	}
	return i
}
...
fn (mut p Parser) array_literal() flat.NodeId {
	...
	return p.a.add_node(flat.Node{
		kind:           .array_literal
		value:          if fixed_literal { 'fixed' } else { '' }
		children_start: start
		children_count: flat.child_count(ids.len)
	})
}
...
fn (mut p Parser) select_branch() flat.NodeId {
	...
	if p.tok == .key_else {
		...
		cond_ids << p.expr_until_block(.lowest)
	...
	if p.tok == .key_fn {
		...
		for p.tok != .rpar && p.tok != .eof {
			...
			if p.tok != .comma && p.tok != .rpar && p.tok != .eof {
				param_type := p.parse_type_name_progress()
				ptypes << if typ.len > 0 {
					if is_mut { 'mut ${typ} ${param_type}' } else { '${typ} ${param_type}' }
				} else {
					if is_mut { 'mut ${param_type}' } else { param_type }
				}
			} else {
				ptypes << if is_mut { 'mut ${typ}' } else { typ }
			}
			if p.tok == .comma {
				p.next()
			}
