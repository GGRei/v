...
fn count_assign_to_ident(a &flat.FlatAst, id flat.NodeId, name string) int {
	if int(id) < 0 {
		return 0
	}
	node := a.nodes[int(id)]
	...
	for i in 0 .. node.children_count {
		total += count_assign_to_ident(a, a.child(&node, i), name)
	}
	return total
}
...
fn test_nested_if_expr_tail_assigns_outer_value_temp() {
	a := parse_transform_source("
fn choose(a bool, b bool, c bool) string {
	x := if a {
		if b {
			if c {
				'ab'
...
")
	rhs := decl_rhs(a, 'choose', 'x')
	assert rhs.kind == .ident
	assert rhs.value.starts_with('__if_val_')
	choose_fn := find_fn(a, 'choose')
	mut assign_count := 0
	for i in 0 .. choose_fn.children_count {
		assign_count += count_assign_to_ident(a, a.child(&choose_fn, i), rhs.value)
	}
	assert assign_count == 4
}

fn test_nested_string_if_expr_array_append_uses_outer_value_temp() {
	a := parse_transform_source("
fn push_param(is_mut bool, typ string, param_type string) {
	mut ptypes := []string{}
	ptypes << if typ.len > 0 {
		if is_mut {
			'mut ' + typ + ' ' + param_type
...
")
	push_fn := find_fn(a, 'push_param')
	mut temp_name := ''
	for i in 0 .. push_fn.children_count {
		child_id := a.child(&push_fn, i)
		node := a.nodes[int(child_id)]
		if node.kind == .decl_assign && node.children_count >= 2 {
			lhs := a.child_node(&node, 0)
			if lhs.kind == .ident && lhs.value.starts_with('__if_val_') {
				temp_name = lhs.value
				break
			}
		}
	}
	assert temp_name.len > 0
	mut assign_count := 0
	mut push_count := 0
	for i in 0 .. push_fn.children_count {
		child_id := a.child(&push_fn, i)
		assign_count += count_assign_to_ident(a, child_id, temp_name)
