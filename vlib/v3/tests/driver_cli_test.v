...
')!
	...
	mut baseline_environment := plain_environment.clone()
	baseline_environment['V3CACHE'] = os.join_path(root, 'plain-baseline-cache')
	mut cached_plain_c := ''
	...
	for iteration in 0 .. 2 {
		...
		if real_pairs.len > 0 {
			...
			plain_baseline := run_driver_with_environment(v3_bin, ['-no-retry-compilation', '-o',
				plain_baseline_output, plain], baseline_environment)
			...
			if iteration == 0 {
				cached_plain_c = plain_c
			} else {
				assert plain_c == cached_plain_c
			}
		}
	}
...
fn test_driver_cpp_linker_uses_final_cpp_driver_and_keeps_sources_c() {
	...
	for index, pair in pairs {
		...
		for mode, mode_args in {
			...
			if mode == 'parallel' {
				assert !c_compile_lines.any(it.contains('src.c')), command_lines.str()
				assert cpp_link_lines[0].count('program_main') == 1, cpp_link_lines[0]
			} else {
				assert c_compile_lines.any(it.contains('src.c') && it.contains('-std=gnu11')), command_lines.str()
				assert cpp_link_lines[0].count('program_main') == 0, cpp_link_lines[0]
			}
			...
			expected_c_objects := (if mode == 'parallel' { 1 } else { 2 }) +
			...
			$if macos {
				...
				objective_c_index := cpp_link_lines[0].index('v3_c_source_2.o') or { -1 }
