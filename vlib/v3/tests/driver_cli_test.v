...
fn test_driver_cpp_linker_uses_final_cpp_driver_and_keeps_sources_c() {
	...
	for index, pair in pairs {
		...
		for mode, mode_args in {
			...
			assert c_compile_lines.any(it.contains('src.c') && it.contains('-std=gnu11')), command_lines.str()
			...
			$if macos {
				...
				assert cpp_compile_lines.any(it.contains('native.mm')), command_lines.str()
			}
			...
			assert cpp_link_lines[0].count('v3_c_source_') == if os.user_os() == 'macos' {
				3
			} else {
				2
			}, cpp_link_lines[0]

			assert cpp_link_lines[0].count('v3_cpp_source_') == if os.user_os() == 'macos' {
				3
			} else {
				2
			}, cpp_link_lines[0]
		...
		}
		{
			...
			assert rsp_cpp_compile_lines.len == if os.user_os() == 'macos' {
				3
			} else {
				2
			}, rsp_command_lines.str()
			...
			if os.user_os() == 'macos' {
				...
				assert rsp_cpp_compile_lines.any(it.contains('native.mm')
					&& it.contains(' -o v3_cpp_source_2.o')), rsp_cpp_compile_lines.str()
			}
			...
			generated_c_index := response_args.index_if(it.contains('v3_c_source_0.o'))
			c_source_index := response_args.index_if(it.contains('v3_c_source_1.o'))
			cpp_source_index := response_args.index_if(it.contains('v3_cpp_source_0.o'))
			cpp_source_second_index := response_args.index_if(it.contains('v3_cpp_source_1.o'))
			runtime_index := response_args.index_if(it.contains('runtime.o'))
			library_index := response_args.index_if(it == '"-lm"')
			...
			if os.user_os() == 'macos' {
				objective_c_index := response_args.index_if(it.contains('v3_c_source_2.o'))
				objective_cpp_index := response_args.index_if(it.contains('v3_cpp_source_2.o'))
			...
			assert response_args.filter(it.contains('v3_cpp_source_')).len == if os.user_os() == 'macos' {
				3
			} else {
				2
			}
