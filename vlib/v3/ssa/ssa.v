...
pub fn (mut m Module) append_phi_operands(instr_idx int, val ValueID, block_id BlockID) {
	...
	if val > 0 && val < m.values.len {
		mut phi_val := ValueID(0)
		if instr.block >= 0 && instr.block < m.blocks.len {
			for candidate in m.blocks[instr.block].instrs {
				if candidate > 0 && candidate < m.values.len
					&& m.values[candidate].kind == .instruction
					&& m.values[candidate].index == instr_idx {
					phi_val = candidate
					break
				}
			}
		}
		if phi_val > 0 && phi_val !in m.values[val].uses {
			mut used_val := m.values[val]
			used_val.uses << phi_val
			m.values[val] = used_val
		}
	}
}
...
pub fn (mut m Module) replace_uses(old_id ValueID, new_id ValueID) {
	if old_id <= 0 || old_id >= m.values.len || new_id <= 0 || new_id >= m.values.len
		|| old_id == new_id {
		return
	}
	for user_id in m.values[old_id].uses.clone() {
		...
		mut changed := false
		for operand_idx in instr.value_operand_positions() {
			if operand_idx >= 0 && operand_idx < instr.operands.len
				&& instr.operands[operand_idx] == old_id {
				instr.operands[operand_idx] = new_id
				changed = true
			}
		}
		if !changed {
		...
		if !still_uses_old {
			...
			for i := old_val.uses.len - 1; i >= 0; i-- {
				if old_val.uses[i] == user_id {
					old_val.uses.delete(i)
				}
			}
...
fn (i &Instruction) value_operand_positions() []int {
	match i.op {
		.br {
			if i.operands.len > 0 {
				return [0]
		...
		.assign {
			...
			mut positions := []int{cap: i.operands.len}
...
pub fn (i &Instruction) value_operands() []ValueID {
	mut values := []ValueID{}
	for operand_idx in i.value_operand_positions() {
		if operand_idx >= 0 && operand_idx < i.operands.len {
			values << i.operands[operand_idx]
		}
	}
	return values
}
