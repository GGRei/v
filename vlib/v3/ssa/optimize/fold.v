	...
	for fi in 0 .. m.funcs.len {
		for blk_id in m.funcs[fi].blocks {
			for val_id in m.blocks[blk_id].instrs {
				...
				if !is_foldable_value_binary_op(instr.op) || instr.operands.len != 2 {
					continue
				}
				lhs_id := instr.operands[0]
				rhs_id := instr.operands[1]
				if lhs_id <= 0 || lhs_id >= m.values.len || rhs_id <= 0 || rhs_id >= m.values.len {
					continue
				}
				lhs := m.values[lhs_id]
				rhs := m.values[rhs_id]
...
fn is_foldable_value_binary_op(op ssa.OpCode) bool {
	return op in [.add, .sub, .mul, .sdiv, .srem, .udiv, .urem, .and_, .or_, .xor, .shl, .ashr,
		.lshr, .eq, .ne, .lt, .gt, .le, .ge, .ult, .ugt, .ule, .uge]
}
