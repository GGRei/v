...
pub:
	// Only cached Value.uses drift is noncritical: all module, CFG, operand,
	// terminator, and phi consistency checks remain fatal.
	allow_noncritical bool
}

fn verify_ssa(m &ssa.Module, stage string) {
	verify_and_panic_with_options(m, stage, VerifyPanicOptions{})
}

pub fn verify_and_panic_with_options(m &ssa.Module, stage string, opts VerifyPanicOptions) {
	verify_module_indexes(m, stage)
	verify_functions(m, stage)
	if !opts.allow_noncritical {
		verify_uses(m, stage)
	}
}
...
fn verify_functions(m &ssa.Module, stage string) {
	for fi in 0 .. m.funcs.len {
		...
		for blk_id in func.blocks {
			...
			verify_phi_cfg_consistency(m, stage, blk_id)
		}
	}
}
...
fn verify_instruction_operands(m &ssa.Module, stage string, fi int, blk_id int, val_id int, instr ssa.Instruction, func_blocks map[int]bool, is_final bool) {
	...
	match instr.op {
		...
		.switch_ {
			if instr.operands.len < 2 || instr.operands.len % 2 != 0 {
				verify_fail(stage,
					'switch value ${val_id} in block ${blk_id} has ${instr.operands.len} operands')
			} else if is_final {
				verify_target_block(m, stage, fi, blk_id, int(instr.operands[1]), func_blocks,
					'switch default')
...
fn verify_phi_cfg_consistency(m &ssa.Module, stage string, blk_id int) {
	blk := m.blocks[blk_id]
	mut expected_preds := []int{cap: blk.preds.len}
	for pred in blk.preds {
		expected_preds << int(pred)
	}
	for val_id in blk.instrs {
		...
		for oi := 1; oi < instr.operands.len; oi += 2 {
			...
			if arr_contains(phi_preds, pred) {
				verify_fail(stage,
					'phi value ${val_id} in block ${blk_id} has duplicate predecessor ${pred}')
			}
			phi_preds << pred
		}
		verify_same_blocks(stage, 'phi predecessors for value ${val_id} in block ${blk_id}',
			expected_preds, phi_preds)
	}
}
...
fn verify_cfg_edges(m &ssa.Module, stage string, fi int, blk_id int, func_blocks map[int]bool) {
	...
	if blk.instrs.len > 0 {
		...
		match term.op {
			...
			.switch_ {
				if term.operands.len >= 2 && term.operands.len % 2 == 0 {
					expected_succs << int(term.operands[1])
					for oi := 3; oi < term.operands.len; oi += 2 {
						if !arr_contains(expected_succs, int(term.operands[oi])) {
							expected_succs << int(term.operands[oi])
						}
					}
				}
			}
