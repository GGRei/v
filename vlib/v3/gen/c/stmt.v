...
fn (mut g FlatGen) gen_node(id flat.NodeId) {
	...
	match node.kind {
		.expr_stmt {
			...
			if child.kind == .call && g.tc.disabled_conditional_call(child_id) {
				return
			}
			...
			if node.children_count > 0 {
				...
				if g.cur_fn_ret_is_optional {
					...
					if base is types.Void {
						...
						if g.option_result_return_expr_has_same_abi(ret_id, expr_type, ct) {
							g.write('return ')
							g.gen_expr(ret_id)
							g.writeln(';')
						} else {
...
fn (mut g FlatGen) option_result_return_expr_has_same_abi(ret_id flat.NodeId, expr_type types.Type, ret_ct string) bool {
	if expr_type is types.OptionType || expr_type is types.ResultType {
		if !g.return_expr_really_has_option_result_abi(ret_id) {
			return false
		}
		return g.optional_type_name(expr_type) == ret_ct
	}
	return false
}

fn (g &FlatGen) return_expr_really_has_option_result_abi(id flat.NodeId) bool {
	if g.expr_really_returns_optional(id) {
		return true
	}
	if int(id) < 0 {
		return false
...
fn (mut g FlatGen) c_type_for_decl_assign(typ types.Type, expr_id flat.NodeId) string {
	if fn_type := fn_type_from(typ) {
		mask := g.tc.expr_fn_const_ptr_param_mask(expr_id)
		return g.resolve_fn_ptr_type(g.fn_ptr_type_key(fn_type, mask))
	}
...
fn (mut g FlatGen) gen_decl_assign(node flat.Node) {
	...
	for i < node.children_count {
		...
		if rhs.kind == .array_literal {
			rhs_type := g.tc.resolve_type(rhs_id)
			if rhs.value == 'fixed' {
				if fixed := array_fixed_type(rhs_type) {
					lhs_str := g.decl_lhs_str(lhs_id)
					g.write('${g.fixed_array_decl(lhs_str, fixed)} = ')
					g.gen_fixed_array_literal_value(rhs, fixed)
					g.writeln(';')
					if lhs.kind == .ident {
						g.tc.cur_scope.insert(lhs.value, rhs_type)
					}
					i += 2
					continue
				}
			...
			if init_type is types.ArrayFixed {
				...
				if len_expr == '0' {
					g.writeln('${g.fixed_array_base_c_type(init_type)} ${lhs_str}[0];')
				} else if init_id := g.fixed_array_init_expr(rhs) {
					g.writeln('${g.fixed_array_decl(lhs_str, init_type)};')
					g.gen_fixed_array_fill(lhs_id, init_id, init_type)
				} else if rhs.children_count == 0 {
					g.writeln('${g.fixed_array_decl(lhs_str, init_type)} = {0};')
				} else {
					g.writeln('${g.fixed_array_decl(lhs_str, init_type)};')
				}
				if lhs.kind == .ident {
			...
			if fixed := array_fixed_type(v_type) {
				lhs_str := g.decl_lhs_str(lhs_id)
				g.writeln('${g.fixed_array_decl(lhs_str, fixed)};')
			...
			ct := g.c_type_for_decl_assign(v_type, rhs_id)
			g.write('${ct} ')
			g.gen_decl_lhs(lhs_id)
			g.write(' = ')
			g.gen_expr(rhs_id)
	...
	for j in 0 .. num_lhs {
		...
		if j < multi_types.len {
			if fixed := array_fixed_type(multi_types[j]) {
				g.writeln('${g.fixed_array_decl(lhs_name, fixed)};')
...
fn (mut g FlatGen) gen_assign(node flat.Node) {
	...
	for i < node.children_count {
		...
		if lhs.kind == .ident && lhs.value == '_' {
			...
			if node.op == .assign {
				if fixed := array_fixed_type(lhs_type) {
					if g.gen_fixed_array_assign(lhs_id, rhs_id, fixed) {
						i += 2
						continue
					}
				}
			...
			if rhs_node.kind == .array_literal
				|| (rhs_node.kind == .array_init && rhs_node.value.len == 0) {
				g.gen_expr(lhs_id)
				g.write(' = ')
...
fn (mut g FlatGen) gen_fixed_array_assign(lhs_id flat.NodeId, rhs_id flat.NodeId, lhs_fixed types.ArrayFixed) bool {
	rhs_node := g.a.nodes[int(rhs_id)]
	c_elem := g.tc.c_type(lhs_fixed.elem_type)
	if rhs_node.kind == .array_literal {
		g.write('memcpy(')
		g.gen_expr_lvalue(lhs_id)
		g.write(', (${c_elem}[]){')
		for j in 0 .. rhs_node.children_count {
			if j > 0 {
				g.write(', ')
			}
	...
	if rhs_node.kind == .array_init {
		if rhs_fixed := array_fixed_type(g.tc.parse_type(rhs_node.value)) {
			if g.fixed_array_len_value(rhs_fixed) == g.fixed_array_len_value(lhs_fixed) {
				if rhs_node.children_count == 0 {
					g.write('memset(')
					g.gen_expr_lvalue(lhs_id)
					g.write(', 0, sizeof(')
					g.gen_expr_lvalue(lhs_id)
				...
				if init_id := g.fixed_array_init_expr(rhs_node) {
					g.gen_fixed_array_fill(lhs_id, init_id, lhs_fixed)
					return true
				}
				return false
			}
	...
	if _ := array_fixed_type(g.tc.resolve_type(rhs_id)) {
		g.write('memmove(')
		g.gen_expr_lvalue(lhs_id)
		g.write(', ')
		g.gen_expr(rhs_id)
...
fn (mut g FlatGen) gen_fixed_array_fill(lhs_id flat.NodeId, init_id flat.NodeId, fixed types.ArrayFixed) {
	idx := g.tmp_name()
	len_expr := g.fixed_array_len_value(fixed)
	g.writeln('for (int ${idx} = 0; ${idx} < ${len_expr}; ${idx}++) {')
	g.indent++
	g.gen_expr_lvalue(lhs_id)
	g.write('[${idx}] = ')
	g.gen_expr(init_id)
