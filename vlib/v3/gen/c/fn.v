...
fn (mut g FlatGen) should_emit_fn_node_in_module(node flat.Node, node_index int, module_name string) bool {
	if !g.used_fn_matches_decl(node.value, module_name) {
		return false
	}
	if g.has_generic_params(flat.NodeId(node_index), node) {
		return false
	}
...
fn (g &FlatGen) used_fn_matches_decl(name string, module_name string) bool {
...
fn (g &FlatGen) module_call_name_for_alias(alias string, fn_name string) string {
	mod := g.modules[alias] or { alias }
	short_mod := if mod.contains('.') {
		mod.all_after_last('.')
	} else {
		mod
	}
	return '${short_mod}.${fn_name}'
}

fn (g &FlatGen) module_member_call_name_for_alias(alias string, member string, fn_name string) string {
	mod := g.modules[alias] or { alias }
	short_mod := if mod.contains('.') {
		mod.all_after_last('.')
	} else {
	...
	return '${short_mod}.${member}.${fn_name}'
}

fn (g &FlatGen) selector_namespace_candidate_matches_resolved(id flat.NodeId, candidate string) bool {
	resolved := g.tc.resolved_call_name(id) or { return false }
	return g.normalize_call_key(resolved) == g.normalize_call_key(candidate)
}
...
fn (mut g FlatGen) gen_fn_in_module(node flat.Node, module_name string) {
	...
	if node.value == 'main' {
		...
		g.write(qfn)
		g.write('(')
		g.write_fn_node_params(node, qfn)
		g.writeln(') {')
	}
	g.indent++
...
fn (mut g FlatGen) gen_fn_value_call(id flat.NodeId, node flat.Node) bool {
	if !g.tc.fn_value_call(id) || node.children_count == 0 {
		return false
	}
	fn_id := g.a.child(&node, 0)
	if int(fn_id) < 0 || int(fn_id) >= g.a.nodes.len {
		return false
	}
	callee_type := g.tc.resolve_type(fn_id)
	fn_type := fn_type_from(callee_type) or { return false }
	g.gen_expr(fn_id)
	g.write('(')
	g.gen_param_aware_args(node, 1, fn_type.params, false, false, '')
	g.write(')')
	return true
}
...
fn (mut g FlatGen) gen_call(id flat.NodeId, node flat.Node) {
	...
	match fn_name {
		...
		'error_with_code' {
			...
			if g.gen_fn_value_call(id, node) {
				return
			}
			mut is_method := false
			mut is_c_call := false
			...
			if fn_node.kind == .selector {
				if g.gen_imported_type_constructor_call(id, node) {
					return
				}
				...
				if base.kind == .ident && base.value == 'C' {
					...
					&& g.selector_namespace_candidate_matches_resolved(id, g.module_call_name_for_alias(base.value, fn_node.value)) {
					full_name := g.module_call_name_for_alias(base.value, fn_node.value)
					if full_name in g.tc.type_aliases || full_name in g.tc.structs
						...
						ct := g.c_type_for_type_constructor(target_type)
					...
					if inner.kind == .ident && inner.value in g.modules
						&& g.selector_namespace_candidate_matches_resolved(id, g.module_member_call_name_for_alias(inner.value, base.value, fn_node.value)) {
						full_name := g.module_member_call_name_for_alias(inner.value, base.value,
							fn_node.value)
						g.write(c_name(full_name))
						g.write('(')
						g.gen_call_args(full_name, node, 1)
						...
						if arr := array_like_type(clean_type) {
							g.gen_array_method_call(id, node, fn_node, arr)
							return
						}
			...
			if param_types.len == 0 && !is_method {
				fn_id := g.a.child(&node, 0)
				callee_type := g.tc.resolve_type(fn_id)
				if callee_fn := fn_type_from(callee_type) {
					param_types = callee_fn.params.clone()
				}
			}
			mut arg_start := 1
			if is_method {
			...
			g.gen_param_aware_args(node, arg_start, param_types, is_method, is_c_call, actual_fn)
			g.write(')')
		}
	}
}

fn (mut g FlatGen) gen_param_aware_args(node flat.Node, arg_start int, param_types []types.Type, is_method bool, is_c_call bool, actual_fn string) {
	for i in arg_start .. node.children_count {
		if is_method || i > arg_start {
			g.write(', ')
		}
		mut arg_idx := int(i) - arg_start
		if is_method {
			arg_idx = int(i)
		}
		...
		if arg_node.kind == .field_init {
			// Trailing `key: value` args form the final struct argument.
			ptyp := if arg_idx < param_types.len {
				param_types[arg_idx]
			} else {
				types.Type(types.void_)
			}
			if !g.gen_named_struct_arg(ptyp, node, i) {
				panic('named arguments require a trailing struct parameter')
			}
			break
		}
...
fn (mut g FlatGen) gen_imported_type_constructor_call(id flat.NodeId, node flat.Node) bool {
	if !g.tc.imported_type_constructor_call(id) || node.children_count == 0 {
		return false
	}
	target_type := g.tc.resolve_type(id)
	target_name := target_type.name()
	if target_name !in g.tc.type_aliases && target_name !in g.tc.structs
		&& target_name !in g.tc.sum_types {
		return false
	}
...
fn (mut g FlatGen) c_type_for_type_constructor(typ types.Type) string {
	if fn_type := fn_type_from(typ) {
		return g.resolve_fn_ptr_type(g.fn_ptr_type_key(fn_type, []bool{}))
	}
...
fn (g &FlatGen) has_generic_params(id flat.NodeId, node flat.Node) bool {
	if g.a.is_generic_template_decl(id) {
		return true
	}
	if g.tc.type_string_has_unresolved_generic_placeholder(node.typ) {
		return true
	}
	for i in 0 .. node.children_count {
		child := g.a.child_node(&node, i)
		if child.kind == .param && g.tc.type_string_has_unresolved_generic_placeholder(child.typ) {
			return true
		}
	}
	return false
...
fn (mut g FlatGen) forward_decls() {
	...
	for i in 0 .. g.a.nodes.len {
		...
		if node.kind == .fn_decl && node.value != 'main' {
			if !g.used_fn_matches_decl(node.value, cur_module) {
				continue
			}
			if g.has_generic_params(flat.NodeId(i), node) {
				continue
			}
			g.tc.cur_module = cur_module
			qfn := qualified_fn_name_in_module(cur_module, node.value)
...
fn (mut g FlatGen) write_fn_node_params(node flat.Node, fn_name string) {
	mut params_len := 0
	for i in 0 .. node.children_count {
	...
	for i in 0 .. node.children_count {
		...
		is_const := fn_const_ptr_mask_value(g.tc.fn_const_ptr_param_mask(fn_name), param_idx)
		ct := if pt is types.OptionType || pt is types.ResultType {
			g.optional_type_name(pt)
		} else {
			g.c_fn_param_type(pt, is_const)
		}
...
fn fn_const_ptr_mask_value(mask []bool, idx int) bool {
	return idx >= 0 && idx < mask.len && mask[idx]
}

fn (g &FlatGen) c_fn_param_type(typ types.Type, is_const bool) string {
	if is_const {
		clean0 := typ
		mut clean := clean0
		if clean0 is types.Alias {
		...
		if clean is types.Pointer {
			return 'const ${g.tc.c_type(clean.base_type)}*'
		}
	}
	return g.tc.c_type(typ)
}
		...
		if ret is types.MultiReturn {
			...
			for i, t in ret.types {
				if fixed := array_fixed_type(t) {
					field_name := 'arg${i}'
					g.writeln('\t${g.fixed_array_decl(field_name, fixed)};')
				} else {
...
fn (mut g FlatGen) fn_ptr_type_key(typ types.FnType, const_mask []bool) string {
	ret := g.fn_ptr_value_c_type(typ.return_type)
	if typ.params.len == 0 {
		return 'fn_ptr:${ret}|void'
	}
	mut params := []string{}
	for i, param in typ.params {
		params << g.fn_ptr_param_c_type(param, fn_const_ptr_mask_value(const_mask, i))
	}
	return 'fn_ptr:${ret}|${params.join(', ')}'
}

fn (mut g FlatGen) fn_ptr_value_c_type(typ types.Type) string {
...
fn (mut g FlatGen) fn_ptr_param_c_type(typ types.Type, is_const bool) string {
	if typ is types.OptionType || typ is types.ResultType {
		return g.optional_type_name(typ)
	}
	return g.c_fn_param_type(typ, is_const)
}
