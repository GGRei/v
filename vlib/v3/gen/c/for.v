...
fn (mut g FlatGen) gen_for_in(node flat.Node) {
	...
	if header_count == 4 {
		...
		if container.kind == .range {
			...
			is_ptr_container := container_type is types.Pointer
			container_str := g.expr_to_string(g.a.child(&node, 2)).trim_space()
			if clean_container_type is types.Map {
				...
				access := if is_ptr_container { '->' } else { '.' }
			...
			} else if clean_container_type is types.Array {
				c_elem := g.tc.c_type(clean_container_type.elem_type)
				len_expr := if is_ptr_container {
					'(${container_str})->len'
				} else {
					'(${container_str}).len'
				}
				array_arg := if is_ptr_container { '*(${container_str})' } else { container_str }
				g.writeln('for (int ${idx_var} = 0; ${idx_var} < ${len_expr}; ${idx_var}++) {')
				g.indent++
				g.writeln('${c_elem} ${elem_var} = *(${c_elem}*)array_get(${array_arg}, ${idx_var});')
				g.tc.cur_scope.insert(elem_var, clean_container_type.elem_type)
			} else if clean_container_type is types.String {
				access := if is_ptr_container { '->' } else { '.' }
				len_expr := '(${container_str})${access}len'
				str_expr := '(${container_str})${access}str'
				g.writeln('for (int ${idx_var} = 0; ${idx_var} < ${len_expr}; ${idx_var}++) {')
				g.indent++
				g.writeln('u8 ${elem_var} = ((u8*)${str_expr})[${idx_var}];')
				g.tc.cur_scope.insert(elem_var, types.Type(types.u8_))
			} else if clean_container_type is types.ArrayFixed {
				af := clean_container_type
				arr_len := g.fixed_array_len_value(af)
				array_expr := if is_ptr_container {
					'(*(${container_str}))'
				} else {
					'(${container_str})'
				}
				...
				g.indent++
				if elem_fixed := array_fixed_type(af.elem_type) {
					g.writeln('${g.fixed_array_decl(elem_var, elem_fixed)};')
					g.writeln('memmove(${elem_var}, ${array_expr}[${idx_var}], sizeof(${elem_var}));')
				} else {
					c_elem := g.tc.c_type(af.elem_type)
					g.writeln('${c_elem} ${elem_var} = ${array_expr}[${idx_var}];')
				}
			...
			if has_index && clean_container_type !is types.Map {
