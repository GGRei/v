...
fn test_fastc_pkgconfig_flags_use_resolved_pkgconfig_runner() {
	...
	$if !windows {
		tool_root = os.join_path(os.vtmp_dir(), 'v3_fastc_pkgconfig_runner_${os.getpid()}')
		os.rmdir_all(tool_root) or {}
...
fn test_source_resolver_uses_compiler_owned_file_defines() {
	...
	parts_dir := os.join_path(root, 'parts')
	os.mkdir_all(module_dir) or { panic(err) }
	os.mkdir_all(parts_dir) or { panic(err) }
	defer {
		os.rmdir_all(root) or {}
	}
	main_file := os.join_path(root, 'main.v')
	...
	os.write_file(os.join_path(root, 'v.mod'), "Module { name: 'main' subdirs: ['parts'] }") or {
		panic(err)
	}
	...
	os.write_file(os.join_path(module_dir, 'variant_notd_v_static_pkgconfig.v'),
		'module variant\npub fn selected() {}\n') or { panic(err) }
	os.write_file(os.join_path(root, 'sibling_d_v_static_pkgconfig.v'), 'module main\n') or {
		panic(err)
	}
	os.write_file(os.join_path(root, 'sibling_notd_v_static_pkgconfig.v'), 'module main\n') or {
		panic(err)
	}
	os.write_file(os.join_path(parts_dir, 'part_d_v_static_pkgconfig.v'), 'module main\n') or {
		panic(err)
	}
	os.write_file(os.join_path(parts_dir, 'part_notd_v_static_pkgconfig.v'), 'module main\n') or {
		panic(err)
	}

	mut prefs := pref.new_preferences()
	prefs.module_search_paths = [root]
	dynamic_sources, _ := fastc_resolve_source_files([main_file], prefs) or { panic(err) }
	...
	dynamic_entry_files := fastc_entry_module_files(main_file, prefs).map(os.base(it))
	assert 'sibling_notd_v_static_pkgconfig.v' in dynamic_entry_files
	assert 'part_notd_v_static_pkgconfig.v' in dynamic_entry_files
	assert 'sibling_d_v_static_pkgconfig.v' !in dynamic_entry_files
	assert 'part_d_v_static_pkgconfig.v' !in dynamic_entry_files

	prefs.file_defines = ['v_static_pkgconfig']
	static_sources, _ := fastc_resolve_source_files([main_file], prefs) or { panic(err) }
	...
	static_entry_files := fastc_entry_module_files(main_file, prefs).map(os.base(it))
	assert 'sibling_d_v_static_pkgconfig.v' in static_entry_files
	assert 'part_d_v_static_pkgconfig.v' in static_entry_files
	assert 'sibling_notd_v_static_pkgconfig.v' !in static_entry_files
	assert 'part_notd_v_static_pkgconfig.v' !in static_entry_files
