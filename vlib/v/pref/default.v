...
// uses_msvc_clang_driver_mode reports Clang's MSVC-compatible command-line dialect.
// It deliberately uses the executable/flag spelling, not an MSYS2 environment name:
// `clang.exe` remains a GNU-compatible driver, while `clang-cl` and an explicit
// `--driver-mode=cl` do not accept the GNU static-link contract used by pkg-config.
