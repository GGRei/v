...
fn (v &Builder) ensure_cpp_linker_is_supported(ccompiler string) {
	...
	if v.ccoptions.cc in [.tcc, .msvc, .emcc] || v.pref.ccompiler_type == .msvc
	...
	cpp_type := resolve_ccompiler_type(cppcompiler, pref.cc_from_string(cppcompiler))
	if cpp_type in [.tinyc, .msvc, .emcc]
		|| compiler_command_uses_msvc_driver_mode(cppcompiler, '', '') {
