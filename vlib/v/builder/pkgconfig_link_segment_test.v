...
fn issue74_assert_direct_cpp_linker_argv(argv []string, root string, expect_flto bool) {
	...
	assert argv[extra_c_index - 2..extra_c_index] == ['-x', 'c']
	assert argv[extra_c_index + 1..extra_c_index + 3] == ['-x', 'none']
	assert generated_c_index >= 2
	generated_language := if os.user_os() == 'macos' { 'objective-c' } else { 'c' }
	assert argv[generated_c_index - 2..generated_c_index] == ['-x', generated_language]
	assert argv[generated_c_index + 1..generated_c_index + 3] == ['-x', 'none']
	...
	marker_object := argv[marker_object_index].trim_space().trim('"').trim("'")
	assert !os.exists(marker_source)
	assert !os.exists(marker_object)
	issue74_assert_cpp_linker_non_probe_drivers(argv)
