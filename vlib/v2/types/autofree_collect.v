...
pub fn (mut e Environment) collect_autofree_facts_from_flat(flat &ast.FlatAst) {
	...
	e.collect_autofree_fresh_string_clone_push_move_proofs_from_flat(flat)
	e.collect_autofree_fresh_string_borrowed_call_move_proofs_from_flat(flat)
...
fn (mut e Environment) autofree_collect_local_array_push_stmt_is_safe(flat &ast.FlatAst, stmt_id ast.FlatNodeId, fresh_name string) bool {
	root_id, rhs_id := autofree_collect_array_push_literal_root_and_rhs_ids(flat, stmt_id) or {
		return false
	}
	...
	if root_name != fresh_name {
		return false
	}
	return e.autofree_collect_local_array_push_rhs_is_safe(flat, rhs_id)
}

fn (mut e Environment) autofree_collect_local_array_push_rhs_is_safe(flat &ast.FlatAst, rhs_id ast.FlatNodeId) bool {
	if !autofree_collect_node_is_valid(flat, rhs_id) {
		return false
	}
	rhs_node := flat.nodes[rhs_id]
	if rhs_node.pos.id <= 0 {
		return false
	}
	if rhs_node.kind != .expr_basic_literal || autofree_collect_exact_edge_count(flat, rhs_id) != 0 {
		return false
	}
	rhs_typ := e.get_expr_type(rhs_node.pos.id) or { return false }
...
fn autofree_collect_array_push_literal_root_and_rhs_ids(flat &ast.FlatAst, stmt_id ast.FlatNodeId) ?(ast.FlatNodeId, ast.FlatNodeId) {
	if root_id, rhs_id := autofree_collect_lowered_array_push_literal_root_and_rhs_ids(flat,
		stmt_id)
	{
		return root_id, rhs_id
	}
	if !autofree_collect_array_push_schema_accepts_literal_rhs(flat, stmt_id) {
		return none
	}
...
struct AutofreeFreshStringClonePushClone {
	receiver_name         string
	receiver_id           ast.FlatNodeId
	receiver_pos_id       int
	call_id               ast.FlatNodeId
	call_pos_id           int
	lowered_wrapper_clone bool
}

struct AutofreeFreshStringClonePushTarget {
	name                string
	id                  ast.FlatNodeId
	pos_id              int
	stmt_id             ast.FlatNodeId
	stmt_pos_id         int
	pointer_destination bool
}

struct AutofreeFreshStringClonePushLoweredArrayTarget {
	id                  ast.FlatNodeId
	pointer_destination bool
}

struct AutofreeFreshStringClonePushFinal {
	target          AutofreeFreshStringClonePushTarget
	clone           AutofreeFreshStringClonePushClone
	temp_name       string
	push_stmt_index int
}

struct AutofreeFreshStringClonePushProof {
	fresh AutofreeFreshLocalFact
	proof AutofreeMoveProofFact
}

struct AutofreeOwnedStringProducer {
	name   string
	reason string
}

struct AutofreeFreshStringBorrowedCallFinal {
	stmt_id     ast.FlatNodeId
	stmt_pos_id int
	call_id     ast.FlatNodeId
	call_pos_id int
	callee_name string
	arg_index   int
	arg_id      ast.FlatNodeId
	arg_pos_id  int
}

struct AutofreeFreshStringBorrowedCallProof {
	fresh AutofreeFreshLocalFact
	proof AutofreeMoveProofFact
}
...
fn (mut e Environment) collect_autofree_fresh_string_clone_push_move_proofs_from_flat(flat &ast.FlatAst) {
	for file in flat.files {
		if !autofree_collect_node_is(flat, file.file_id, .file) {
			continue
		}
		...
		for i in 0 .. stmt_count {
			...
			e.collect_autofree_fresh_string_clone_push_move_proofs_from_fn(flat, module_name, fn_id)
		}
	}
}

fn (mut e Environment) collect_autofree_fresh_string_clone_push_move_proofs_from_fn(flat &ast.FlatAst, module_name string, fn_id ast.FlatNodeId) {
	if !autofree_collect_node_is(flat, fn_id, .stmt_fn_decl)
		|| !autofree_collect_fn_schema_is_complete(flat, fn_id) {
		return
	}
	fn_node := flat.nodes[fn_id]
	if (fn_node.flags & ast.flag_is_method) != 0 || unsafe { ast.Language(int(fn_node.aux)) } != .v {
		return
	}
	fn_name := flat.string_at(fn_node.name_id)
	if fn_name.len == 0 {
...
fn (mut e Environment) collect_autofree_fresh_string_clone_push_fresh_from_stmt(flat &ast.FlatAst, fn_key string, fn_name string, stmt_id ast.FlatNodeId, param_names map[string]bool) ?AutofreeFreshLocalFact {
	...
	rhs_pos_id := flat.nodes[rhs_id].pos.id
	if lhs_pos_id <= 0 || rhs_pos_id <= 0
		|| !e.autofree_collect_fresh_string_clone_push_plus_call_is_owned(flat, rhs_id, lhs_name, param_names) {
		return none
	}
	lhs_typ := e.get_expr_type(lhs_pos_id) or { return none }
	rhs_typ := e.get_expr_type(rhs_pos_id) or { return none }
	if !type_has_valid_payload(lhs_typ) || !type_has_valid_payload(rhs_typ)
		|| !autofree_collect_builtin_string_equivalent_type(lhs_typ)
		|| !autofree_collect_builtin_string_equivalent_type(rhs_typ) {
		return none
	}
	fact_typ := Type(string_)
	shape := e.autofree_resource_shape(fact_typ)
	if !autofree_collect_fresh_string_cleanup_shape_allowed(shape) {
		return none
	}
	endpoint := autofree_collect_fresh_local_endpoint(flat, lhs_id, lhs_name, fact_typ, shape)
	source_endpoint := AutofreeTransferEndpoint{
		...autofree_collect_endpoint(flat, rhs_id, rhs_pos_id, rhs_id, 'string__plus', fact_typ,
			shape, .call_result, 'owned string plus call result')
		state: .owned_unique
	}
	return AutofreeFreshLocalFact{
		fn_key:          fn_key
...
fn (mut e Environment) autofree_collect_fresh_string_clone_push_plus_call_is_owned(flat &ast.FlatAst, node_id ast.FlatNodeId, target_name string, param_names map[string]bool) bool {
	if !autofree_collect_node_is(flat, node_id, .expr_call)
		|| autofree_collect_exact_edge_count(flat, node_id) != 3 {
		return false
	}
	call_node := flat.nodes[node_id]
	if call_node.pos.id <= 0 {
		return false
	}
	callee_id := flat.edges[call_node.first_edge].child_id
	if !autofree_collect_node_is(flat, callee_id, .expr_ident)
		|| autofree_collect_exact_edge_count(flat, callee_id) != 0
		|| flat.string_at(flat.nodes[callee_id].name_id) != 'string__plus' {
		return false
	}
	for i in 1 .. 3 {
		arg_id := flat.edges[call_node.first_edge + i].child_id
		if !autofree_collect_node_is(flat, arg_id, .expr_ident)
			|| autofree_collect_exact_edge_count(flat, arg_id) != 0 {
			return false
		}
		arg_name := flat.string_at(flat.nodes[arg_id].name_id)
		if !autofree_collect_name_is_usable(arg_name) || arg_name == target_name
			|| arg_name !in param_names {
			return false
		}
		arg_typ := e.get_expr_type(flat.nodes[arg_id].pos.id) or { return false }
		if !type_has_valid_payload(arg_typ)
			|| !autofree_collect_builtin_string_equivalent_type(arg_typ) {
			return false
		}
	}
	return true
...
fn autofree_collect_fresh_string_clone_push_final_from_body(flat &ast.FlatAst, body_id ast.FlatNodeId, body_count int, fresh_stmt_index int, local_name string) ?AutofreeFreshStringClonePushFinal {
	...
	target, clone := autofree_collect_fresh_string_clone_push_lowered_final_stmt(flat,
		source_push_stmt_id, local_name) or { return none }
	return AutofreeFreshStringClonePushFinal{
		target:          target
		clone:           clone
		push_stmt_index: push_stmt_index
...
fn autofree_collect_fresh_string_clone_push_source_final_stmt(flat &ast.FlatAst, stmt_id ast.FlatNodeId, local_name string) ?(AutofreeFreshStringClonePushTarget, AutofreeFreshStringClonePushClone) {
	if !autofree_collect_node_is(flat, stmt_id, .stmt_expr)
		|| autofree_collect_exact_edge_count(flat, stmt_id) != 1 {
		return none
	}
	...
	lhs_id := flat.edges[flat.nodes[push_id].first_edge].child_id
	rhs_id := flat.edges[flat.nodes[push_id].first_edge + 1].child_id
	target := autofree_collect_fresh_string_clone_push_direct_array_target(flat, lhs_id, stmt_id,
		flat.nodes[push_id].pos.id, false) or { return none }
	clone := autofree_collect_fresh_string_clone_push_clone_rhs(flat, rhs_id, local_name) or {
		return none
	}
	return target, clone
}

fn autofree_collect_fresh_string_clone_push_lowered_final_stmt(flat &ast.FlatAst, stmt_id ast.FlatNodeId, local_name string) ?(AutofreeFreshStringClonePushTarget, AutofreeFreshStringClonePushClone) {
	if !autofree_collect_node_is(flat, stmt_id, .stmt_expr)
		|| autofree_collect_exact_edge_count(flat, stmt_id) != 1 {
		return none
	}
	...
	arr_ptr_id := flat.edges[call_node.first_edge + 1].child_id
	lowered_target := autofree_collect_fresh_string_clone_push_lowered_array_target(flat,
		arr_ptr_id) or { return none }
	target := autofree_collect_fresh_string_clone_push_direct_array_target(flat, lowered_target.id,
		stmt_id, call_node.pos.id, lowered_target.pointer_destination) or { return none }
	wrapper_id := flat.edges[call_node.first_edge + 2].child_id
	clone := autofree_collect_fresh_string_clone_push_lowered_single_wrapper_clone(flat,
		wrapper_id, local_name) or { return none }
	return target, clone
}

fn autofree_collect_fresh_string_clone_push_direct_array_target(flat &ast.FlatAst, target_id ast.FlatNodeId, stmt_id ast.FlatNodeId, stmt_pos_id int, pointer_destination bool) ?AutofreeFreshStringClonePushTarget {
	if !autofree_collect_node_is(flat, target_id, .expr_ident)
		|| autofree_collect_exact_edge_count(flat, target_id) != 0 {
		return none
	}
	target_name := flat.string_at(flat.nodes[target_id].name_id)
	target_pos_id := flat.nodes[target_id].pos.id
	if !autofree_collect_name_is_usable(target_name) || target_pos_id <= 0 || stmt_pos_id <= 0 {
		return none
	}
	return AutofreeFreshStringClonePushTarget{
		name:                target_name
		id:                  target_id
		pos_id:              target_pos_id
		stmt_id:             stmt_id
		stmt_pos_id:         stmt_pos_id
		pointer_destination: pointer_destination
	}
}

fn autofree_collect_fresh_string_clone_push_lowered_array_target(flat &ast.FlatAst, arr_ptr_id ast.FlatNodeId) ?AutofreeFreshStringClonePushLoweredArrayTarget {
	if !autofree_collect_node_is(flat, arr_ptr_id, .expr_cast)
		|| autofree_collect_exact_edge_count(flat, arr_ptr_id) != 2 {
		return none
	}
	...
	inner_id := flat.edges[flat.nodes[arr_ptr_id].first_edge + 1].child_id
	if autofree_collect_node_is(flat, inner_id, .expr_ident)
		&& autofree_collect_exact_edge_count(flat, inner_id) == 0 {
		return AutofreeFreshStringClonePushLoweredArrayTarget{
			id:                  inner_id
			pointer_destination: true
		}
	}
	if !autofree_collect_node_is(flat, inner_id, .expr_prefix)
		|| autofree_collect_exact_edge_count(flat, inner_id) != 1
		|| unsafe { token.Token(int(flat.nodes[inner_id].aux)) } != .amp
		|| !autofree_collect_child_is_valid(flat, inner_id, 0) {
		return none
	}
	target_id := flat.edges[flat.nodes[inner_id].first_edge].child_id
	if !autofree_collect_node_is(flat, target_id, .expr_ident) {
		return none
	}
	return AutofreeFreshStringClonePushLoweredArrayTarget{
...
fn autofree_collect_fresh_string_clone_push_lowered_single_wrapper_clone(flat &ast.FlatAst, wrapper_id ast.FlatNodeId, local_name string) ?AutofreeFreshStringClonePushClone {
	if !autofree_collect_node_is(flat, wrapper_id, .expr_array_init)
		|| autofree_collect_exact_edge_count(flat, wrapper_id) != 6 {
		return none
	}
	clone_id := flat.edges[flat.nodes[wrapper_id].first_edge + 5].child_id
	if !autofree_collect_node_is(flat, clone_id, .expr_call)
		|| autofree_collect_exact_edge_count(flat, clone_id) != 2 {
		return none
	}
	return autofree_collect_fresh_string_clone_push_lowered_wrapper_clone_rhs(flat, clone_id,
		local_name)
}

fn autofree_collect_fresh_string_clone_push_lowered_wrapper_clone_rhs(flat &ast.FlatAst, rhs_id ast.FlatNodeId, local_name string) ?AutofreeFreshStringClonePushClone {
	if !autofree_collect_node_is(flat, rhs_id, .expr_call) {
		return none
	}
	call_node := flat.nodes[rhs_id]
	if call_node.pos.id != 0 || autofree_collect_exact_edge_count(flat, rhs_id) != 2 {
		return none
	}
	callee_id := flat.edges[call_node.first_edge].child_id
	receiver_id := flat.edges[call_node.first_edge + 1].child_id
	if !autofree_collect_node_is(flat, callee_id, .expr_ident)
		...
		|| autofree_collect_exact_edge_count(flat, callee_id) != 0
		|| autofree_collect_exact_edge_count(flat, receiver_id) != 0
		|| flat.string_at(flat.nodes[callee_id].name_id) != 'string__clone' {
		return none
	}
	receiver_name := flat.string_at(flat.nodes[receiver_id].name_id)
	if receiver_name != local_name {
	...
	return AutofreeFreshStringClonePushClone{
		receiver_name:         receiver_name
		receiver_id:           receiver_id
		receiver_pos_id:       flat.nodes[receiver_id].pos.id
		call_id:               rhs_id
		call_pos_id:           call_node.pos.id
		lowered_wrapper_clone: true
	}
}

fn autofree_collect_fresh_string_clone_push_clone_rhs(flat &ast.FlatAst, rhs_id ast.FlatNodeId, local_name string) ?AutofreeFreshStringClonePushClone {
	if !autofree_collect_node_is(flat, rhs_id, .expr_call) {
		return none
	}
	call_node := flat.nodes[rhs_id]
...
fn autofree_collect_fresh_string_clone_push_tail_is_safe(flat &ast.FlatAst, body_id ast.FlatNodeId, body_count int, push_stmt_index int, local_name string, temp_name string) bool {
	if !autofree_collect_node_is(flat, body_id, .aux_list) || push_stmt_index < 1
		|| push_stmt_index >= body_count {
		return false
	}
	if push_stmt_index == body_count - 1 {
		return true
	}
	if push_stmt_index != body_count - 2 {
		return false
	}
	body_node := flat.nodes[body_id]
	tail_stmt_id := flat.edges[body_node.first_edge + body_count - 1].child_id
	if !autofree_collect_node_is(flat, tail_stmt_id, .stmt_return) {
		return false
	}
	return !autofree_collect_subtree_contains_ident(flat, tail_stmt_id, local_name)
		&& (temp_name.len == 0
...
fn autofree_collect_fresh_string_clone_push_destination_accepts(dest_typ Type, item_typ Type, pointer_destination bool) bool {
	...
	if !same_type_name(item_typ, string_type) {
		return false
	}
	if same_type_name(dest_typ, string_array_type) {
		return !pointer_destination
	}
	if !pointer_destination {
		return false
	}
	if dest_typ is Pointer {
		return same_type_name(dest_typ.base_type, string_array_type)
	}
	return false
}
...
fn autofree_collect_fresh_string_clone_push_move_proof_from_fresh(fresh AutofreeFreshLocalFact) ?AutofreeMoveProofFact {
	source := fresh.source_endpoint
	target := fresh.endpoint
	if fresh.fn_key.len == 0 || fresh.name.len == 0 || fresh.stmt_node_id < 0
		|| fresh.stmt_pos_id <= 0 {
		return none
	}
	if !autofree_collect_fresh_string_clone_push_source_endpoint_is_allowed(source) {
		return none
	}
	if target.storage != .local || target.root_storage != .local || target.path.len != 0 {
		return none
	...
	if !source.has_type || !target.has_type || !type_has_valid_payload(source.typ)
		|| !type_has_valid_payload(target.typ) || !type_has_valid_payload(fresh.typ)
		|| !same_type_name(source.typ, target.typ) || !same_type_name(target.typ, fresh.typ) {
		return none
	}
	canonical_type_name := fresh.typ.name()
	if canonical_type_name.len == 0 || fresh.type_name.len == 0 || source.type_name.len == 0
	...
	if fresh.resource != .string_value || target.resource != .string_value
		|| source.resource != .string_value {
		return none
	}
	shape := fresh.shape
	if !autofree_collect_fresh_string_cleanup_shape_allowed(shape)
	...
	return AutofreeMoveProofFact{
		...
		stmt_node_id:    fresh.stmt_node_id
		stmt_pos_id:     fresh.stmt_pos_id
		reason:          'fresh local string clone push binding'
	}
}

fn autofree_collect_fresh_string_cleanup_shape_allowed(shape AutofreeResourceShape) bool {
	return shape.kind == .string_ && !shape.fail_closed && shape.needs_autofree()
}

fn (mut e Environment) collect_autofree_fresh_string_borrowed_call_move_proofs_from_flat(flat &ast.FlatAst) {
	for file in flat.files {
		if !autofree_collect_node_is(flat, file.file_id, .file) {
			continue
		}
		...
		for i in 0 .. stmt_count {
			...
			e.collect_autofree_fresh_string_borrowed_call_move_proofs_from_fn(flat, module_name,
				fn_id)
		}
	}
}

fn (mut e Environment) collect_autofree_fresh_string_borrowed_call_move_proofs_from_fn(flat &ast.FlatAst, module_name string, fn_id ast.FlatNodeId) {
	if !autofree_collect_node_is(flat, fn_id, .stmt_fn_decl)
		|| !autofree_collect_fn_schema_is_complete(flat, fn_id) {
		return
	}
	...
	mut proofs := e.autofree_move_proofs_by_fn_key[fn_key] or { []AutofreeMoveProofFact{} }
	mut changed := false
	for fresh_stmt_index in 0 .. body_count - 1 {
		string_proof := e.collect_autofree_fresh_string_borrowed_call_move_proof_from_window(flat,
			module_name, fn_key, fn_name, body_id, body_count, fresh_stmt_index, param_names) or {
			continue
		}
		fresh_locals << string_proof.fresh
		proofs << string_proof.proof
...
fn (mut e Environment) collect_autofree_fresh_string_borrowed_call_move_proof_from_window(flat &ast.FlatAst, module_name string, fn_key string, fn_name string, body_id ast.FlatNodeId, body_count int, fresh_stmt_index int, param_names map[string]bool) ?AutofreeFreshStringBorrowedCallProof {
	if !autofree_collect_node_is(flat, body_id, .aux_list) || fresh_stmt_index < 0
		|| fresh_stmt_index + 1 >= body_count {
		return none
	}
	body_node := flat.nodes[body_id]
	fresh_stmt_id := flat.edges[body_node.first_edge + fresh_stmt_index].child_id
	fresh := e.collect_autofree_fresh_string_borrowed_call_fresh_from_stmt(flat, fn_key, fn_name,
		fresh_stmt_id, param_names) or { return none }
	final := e.autofree_collect_fresh_string_borrowed_call_final_from_body(flat, module_name,
		body_id, body_count, fresh_stmt_index, fresh) or { return none }
	if !autofree_collect_fresh_string_borrowed_call_tail_is_safe(flat, body_id, body_count,

		fresh_stmt_index + 1, fresh.name) {
		return none
	}
	if final.arg_id < 0 || final.arg_pos_id <= 0 || final.call_pos_id <= 0 || final.stmt_pos_id <= 0 {
		return none
	}
	arg_typ := e.get_expr_type(final.arg_pos_id) or { return none }
	if !type_has_valid_payload(arg_typ) || !same_type_name(arg_typ, fresh.typ) {
		return none
	}
	proof := autofree_collect_fresh_string_borrowed_call_move_proof_from_fresh(fresh) or {
		return none
	}
	return AutofreeFreshStringBorrowedCallProof{
		fresh: fresh
		proof: AutofreeMoveProofFact{
			...proof
			stmt_node_id: final.stmt_id
			stmt_pos_id:  final.stmt_pos_id
			reason:       'fresh local string borrowed call cleanup'
		}
	}
}

fn (mut e Environment) collect_autofree_fresh_string_borrowed_call_fresh_from_stmt(flat &ast.FlatAst, fn_key string, fn_name string, stmt_id ast.FlatNodeId, param_names map[string]bool) ?AutofreeFreshLocalFact {
	if !autofree_collect_node_is(flat, stmt_id, .stmt_assign) {
		return none
	}
	stmt_node := flat.nodes[stmt_id]
	...
	producer := e.autofree_collect_owned_string_producer(flat, rhs_id, lhs_name, param_names) or {
		return none
	}
	lhs_typ := e.get_expr_type(lhs_pos_id) or { return none }
	rhs_typ := e.get_expr_type(rhs_pos_id) or { return none }
	...
	endpoint := autofree_collect_fresh_local_endpoint(flat, lhs_id, lhs_name, fact_typ, shape)
	source_endpoint := AutofreeTransferEndpoint{
		...autofree_collect_endpoint(flat, rhs_id, rhs_pos_id, rhs_id, producer.name, fact_typ,
			shape, .call_result, producer.reason)
		state: .owned_unique
	}
	return AutofreeFreshLocalFact{
		fn_key:          fn_key
...
fn (mut e Environment) autofree_collect_owned_string_producer(flat &ast.FlatAst, node_id ast.FlatNodeId, target_name string, param_names map[string]bool) ?AutofreeOwnedStringProducer {
	if e.autofree_collect_fresh_string_clone_push_plus_call_is_owned(flat, node_id, target_name,
		param_names)
	{
		return AutofreeOwnedStringProducer{
			name:   'string__plus'
			reason: 'owned string plus call result'
		}
	}
	name := e.autofree_collect_owned_string_str_call_name(flat, node_id, target_name) or {
		return none
	}
	return AutofreeOwnedStringProducer{
		name:   name
		reason: 'owned string str call result'
	}
}

fn (mut e Environment) autofree_collect_owned_string_str_call_name(flat &ast.FlatAst, node_id ast.FlatNodeId, target_name string) ?string {
	if !autofree_collect_node_is(flat, node_id, .expr_call)
		|| autofree_collect_exact_edge_count(flat, node_id) != 2 {
		return none
	}
	call_node := flat.nodes[node_id]
	if call_node.pos.id <= 0 {
	...
	callee_id := flat.edges[call_node.first_edge].child_id
	arg_id := flat.edges[call_node.first_edge + 1].child_id
	if !autofree_collect_node_is(flat, callee_id, .expr_ident)
		|| !autofree_collect_node_is(flat, arg_id, .expr_ident)
		|| autofree_collect_exact_edge_count(flat, callee_id) != 0
		|| autofree_collect_exact_edge_count(flat, arg_id) != 0 {
		return none
	}
	callee_name := flat.string_at(flat.nodes[callee_id].name_id)
	if callee_name !in ['int__str', 'i64__str', 'u64__str'] {
		return none
	}
	arg_name := flat.string_at(flat.nodes[arg_id].name_id)
	if !autofree_collect_name_is_usable(arg_name) || arg_name == target_name {
	...
	arg_pos_id := flat.nodes[arg_id].pos.id
	if arg_pos_id <= 0 {
		return none
	}
	arg_typ := e.get_expr_type(arg_pos_id) or { return none }
	if !type_has_valid_payload(arg_typ) || (!arg_typ.is_int_literal() && !arg_typ.is_integer()) {
		return none
	}
	arg_shape := e.autofree_resource_shape(arg_typ)
	if arg_shape.fail_closed || arg_shape.kind != .no_resource {
		return none
	}
	return callee_name
}

fn (mut e Environment) autofree_collect_fresh_string_borrowed_call_final_from_body(flat &ast.FlatAst, module_name string, body_id ast.FlatNodeId, body_count int, fresh_stmt_index int, fresh AutofreeFreshLocalFact) ?AutofreeFreshStringBorrowedCallFinal {
	if fresh_stmt_index < 0 || fresh_stmt_index + 1 >= body_count
		|| !autofree_collect_node_is(flat, body_id, .aux_list) {
		return none
	}
	body_node := flat.nodes[body_id]
	call_stmt_index := fresh_stmt_index + 1
	call_stmt_id := flat.edges[body_node.first_edge + call_stmt_index].child_id
	final := autofree_collect_fresh_string_borrowed_call_final_stmt(flat, call_stmt_id, fresh.name) or {
		return none
	}
	if !e.autofree_collect_borrowed_string_callee_is_non_escaping(flat, module_name,
		final.callee_name, final.arg_index) {
		return none
	}
	return final
}

fn autofree_collect_fresh_string_borrowed_call_final_stmt(flat &ast.FlatAst, stmt_id ast.FlatNodeId, local_name string) ?AutofreeFreshStringBorrowedCallFinal {
	if !autofree_collect_node_is(flat, stmt_id, .stmt_expr)
		|| autofree_collect_exact_edge_count(flat, stmt_id) != 1 {
		return none
	}
	call_id := flat.edges[flat.nodes[stmt_id].first_edge].child_id
	if !autofree_collect_node_is(flat, call_id, .expr_call) {
		return none
	}
	call_node := flat.nodes[call_id]
	edge_count := autofree_collect_exact_edge_count(flat, call_id)
	if edge_count < 2 || call_node.pos.id <= 0 {
		return none
	}
	callee_id := flat.edges[call_node.first_edge].child_id
	if !autofree_collect_node_is(flat, callee_id, .expr_ident)
	...
	callee_name := flat.string_at(flat.nodes[callee_id].name_id)
	if !autofree_collect_name_is_usable(callee_name) {
		return none
	}
	mut arg_count := 0
	mut final_arg_index := -1
	mut final_arg_id := ast.invalid_flat_node_id
	mut final_arg_pos_id := 0
	for i in 1 .. edge_count {
		arg_id := flat.edges[call_node.first_edge + i].child_id
		if !autofree_collect_subtree_contains_ident(flat, arg_id, local_name) {
			continue
		}
		if !autofree_collect_node_is(flat, arg_id, .expr_ident)
			|| autofree_collect_exact_edge_count(flat, arg_id) != 0
			|| flat.string_at(flat.nodes[arg_id].name_id) != local_name {
			return none
		}
		arg_count++
		final_arg_index = i - 1
		final_arg_id = arg_id
		final_arg_pos_id = flat.nodes[arg_id].pos.id
	}
	if arg_count != 1 || final_arg_id == ast.invalid_flat_node_id || final_arg_pos_id <= 0 {
		return none
	}
	return AutofreeFreshStringBorrowedCallFinal{
		stmt_id:     stmt_id
		stmt_pos_id: call_node.pos.id
		call_id:     call_id
		call_pos_id: call_node.pos.id
		...
		arg_id:      final_arg_id
		arg_pos_id:  final_arg_pos_id
	}
}

fn (mut e Environment) autofree_collect_borrowed_string_callee_is_non_escaping(flat &ast.FlatAst, module_name string, callee_name string, arg_index int) bool {
	callee_id := autofree_collect_direct_v_fn_decl_by_name(flat, module_name, callee_name) or {
		return false
	}
	param_ids := autofree_collect_fn_param_ids(flat, callee_id)
	if arg_index < 0 || arg_index >= param_ids.len {
		return false
	}
	param_id := param_ids[arg_index]
	if (flat.nodes[param_id].flags & ast.flag_is_mut) != 0 {
		return false
	}
	param_name := flat.string_at(flat.nodes[param_id].name_id)
	if !autofree_collect_name_is_usable(param_name) {
		return false
	}
	param_typ := e.autofree_collect_param_type(flat, param_id) or { return false }
	if !type_has_valid_payload(param_typ)
		|| !autofree_collect_builtin_string_equivalent_type(param_typ) {
		return false
	}
	body_id := autofree_collect_required_child(flat, callee_id, 3, .aux_list)
	if body_id == ast.invalid_flat_node_id {
		return false
	}
	body_count := autofree_collect_exact_edge_count(flat, body_id)
	...
	body_node := flat.nodes[body_id]
	if body_count > 0
		&& (body_node.first_edge < 0 || body_node.first_edge + body_count > flat.edges.len) {
		return false
	}
	for i in 0 .. body_count {
		stmt_id := flat.edges[body_node.first_edge + i].child_id
		if !autofree_collect_borrowed_string_param_stmt_is_read_only(flat, stmt_id, param_name) {
			return false
		}
	}
	return true
...
fn autofree_collect_direct_v_fn_decl_by_name(flat &ast.FlatAst, module_name string, fn_name string) ?ast.FlatNodeId {
	if !autofree_collect_name_is_usable(fn_name) {
		return none
	}
	mut found := ast.invalid_flat_node_id
	for file in flat.files {
		if !autofree_collect_node_is(flat, file.file_id, .file)
			|| flat.string_at(file.mod_idx) != module_name {
			continue
		}
		stmts_id := autofree_collect_required_child(flat, file.file_id, 2, .aux_list)
		if stmts_id == ast.invalid_flat_node_id {
		...
		for i in 0 .. stmt_count {
			...
			if (fn_node.flags & ast.flag_is_method) != 0
				|| unsafe { ast.Language(int(fn_node.aux)) } != .v
				|| flat.string_at(fn_node.name_id) != fn_name {
				continue
			}
			if found != ast.invalid_flat_node_id {
				return none
			}
			found = fn_id
		}
...
fn (mut e Environment) autofree_collect_param_type(flat &ast.FlatAst, param_id ast.FlatNodeId) ?Type {
	if !autofree_collect_node_is(flat, param_id, .aux_parameter)
		|| !autofree_collect_parameter_schema_is_complete(flat, param_id) {
		return none
	}
	param_pos_id := flat.nodes[param_id].pos.id
	if param_pos_id > 0 {
		if typ := e.get_expr_type(param_pos_id) {
			if type_has_valid_payload(typ) {
				return typ
			}
		}
	}
	type_id := flat.edges[flat.nodes[param_id].first_edge].child_id
	type_pos_id := flat.nodes[type_id].pos.id
	if type_pos_id > 0 {
		if typ := e.get_expr_type(type_pos_id) {
			if type_has_valid_payload(typ) {
				return typ
			}
		}
	...
	if autofree_collect_node_is(flat, type_id, .expr_ident)
		...
		type_name := flat.string_at(flat.nodes[type_id].name_id)
		if type_name == 'string' {
			return Type(string_)
		}
	}
...
fn autofree_collect_borrowed_string_param_stmt_is_read_only(flat &ast.FlatAst, stmt_id ast.FlatNodeId, param_name string) bool {
	if !autofree_collect_node_is_valid(flat, stmt_id) || param_name.len == 0 {
		return false
	}
	if !autofree_collect_subtree_contains_ident(flat, stmt_id, param_name) {
		return true
	}
	if autofree_collect_node_is(flat, stmt_id, .stmt_assign) {
		return autofree_collect_borrowed_string_param_assign_is_read_only(flat, stmt_id, param_name)
	}
	if autofree_collect_node_is(flat, stmt_id, .stmt_expr)
		&& autofree_collect_exact_edge_count(flat, stmt_id) == 1 {
		expr_id := flat.edges[flat.nodes[stmt_id].first_edge].child_id
		return autofree_collect_borrowed_string_param_expr_is_read_only(flat, expr_id, param_name)
	}
	return false
}

fn autofree_collect_borrowed_string_param_assign_is_read_only(flat &ast.FlatAst, stmt_id ast.FlatNodeId, param_name string) bool {
	if !autofree_collect_node_is(flat, stmt_id, .stmt_assign) {
		return false
	}
	stmt_node := flat.nodes[stmt_id]
	...
	if !autofree_collect_node_is(flat, lhs_id, .expr_ident)
		|| autofree_collect_exact_edge_count(flat, lhs_id) != 0
		|| flat.string_at(flat.nodes[lhs_id].name_id) != '_' {
		return false
	}
	return autofree_collect_borrowed_string_param_expr_is_read_only(flat, rhs_id, param_name)
}

fn autofree_collect_borrowed_string_param_expr_is_read_only(flat &ast.FlatAst, expr_id ast.FlatNodeId, param_name string) bool {
	if !autofree_collect_node_is_valid(flat, expr_id) || param_name.len == 0 {
		return false
	}
	if autofree_collect_node_is(flat, expr_id, .expr_ident)
		&& autofree_collect_exact_edge_count(flat, expr_id) == 0 {
		return flat.string_at(flat.nodes[expr_id].name_id) == param_name
	}
	if autofree_collect_node_is(flat, expr_id, .expr_selector)
		&& autofree_collect_exact_edge_count(flat, expr_id) == 2 {
		root_id := flat.edges[flat.nodes[expr_id].first_edge].child_id
		field_id := flat.edges[flat.nodes[expr_id].first_edge + 1].child_id
		return autofree_collect_node_is(flat, root_id, .expr_ident)
			&& autofree_collect_exact_edge_count(flat, root_id) == 0
			&& flat.string_at(flat.nodes[root_id].name_id) == param_name
			&& autofree_collect_node_is(flat, field_id, .expr_ident)
			&& autofree_collect_exact_edge_count(flat, field_id) == 0
	}
	return false
...
fn autofree_collect_fresh_string_borrowed_call_tail_is_safe(flat &ast.FlatAst, body_id ast.FlatNodeId, body_count int, call_stmt_index int, local_name string) bool {
	if !autofree_collect_node_is(flat, body_id, .aux_list) || call_stmt_index < 1
		|| call_stmt_index >= body_count || local_name.len == 0 {
		return false
	}
	body_node := flat.nodes[body_id]
	for i in call_stmt_index + 1 .. body_count {
		stmt_id := flat.edges[body_node.first_edge + i].child_id
		if autofree_collect_subtree_contains_ident(flat, stmt_id, local_name) {
			return false
		}
	}
	return true
...
fn autofree_collect_fresh_string_borrowed_call_move_proof_from_fresh(fresh AutofreeFreshLocalFact) ?AutofreeMoveProofFact {
	source := fresh.source_endpoint
	target := fresh.endpoint
	if fresh.fn_key.len == 0 || fresh.name.len == 0 || fresh.stmt_node_id < 0
		|| fresh.stmt_pos_id <= 0 {
	...
	if !autofree_collect_fresh_string_borrowed_call_source_endpoint_is_allowed(source) {
		return none
	}
	if target.storage != .local || target.root_storage != .local || target.path.len != 0 {
		return none
	...
	return AutofreeMoveProofFact{
		...
		stmt_node_id:    fresh.stmt_node_id
		stmt_pos_id:     fresh.stmt_pos_id
		reason:          'fresh local string borrowed call binding'
	}
}
...
fn (mut e Environment) collect_autofree_fresh_array_final_clone_move_proof_from_fn(flat &ast.FlatAst, module_name string, fn_id ast.FlatNodeId) {
	...
	proof := if body_count == 2 {
		...
		e.collect_autofree_fresh_array_final_clone_move_proof_from_stmts(flat, fn_key, fn_name,
			fresh_stmt_id, final_stmt_id, param_names, param_ids_by_name) or { return }
	} else {
		prefix_stmt_id := flat.edges[body_node.first_edge].child_id
		fresh_stmt_id := flat.edges[body_node.first_edge + 1].child_id
		final_stmt_id := flat.edges[body_node.first_edge + 2].child_id
		e.collect_autofree_prefixed_fresh_array_final_clone_move_proof_from_stmts(flat, fn_key,
			fn_name, prefix_stmt_id, fresh_stmt_id, final_stmt_id, param_names, param_ids_by_name) or {
			return
		}
	}
	mut proofs := e.autofree_move_proofs_by_fn_key[proof.fn_key] or { []AutofreeMoveProofFact{} }
...
fn (mut e Environment) collect_autofree_fresh_array_final_clone_move_proof_from_stmts(flat &ast.FlatAst, fn_key string, fn_name string, fresh_stmt_id ast.FlatNodeId, final_stmt_id ast.FlatNodeId, param_names map[string]bool, param_ids_by_name map[string]ast.FlatNodeId) ?AutofreeMoveProofFact {
	...
	return e.collect_autofree_fresh_array_final_clone_move_proof_from_fresh(flat, fresh,
		final_stmt_id, param_names, param_ids_by_name)
}

fn (mut e Environment) collect_autofree_prefixed_fresh_array_final_clone_move_proof_from_stmts(flat &ast.FlatAst, fn_key string, fn_name string, prefix_stmt_id ast.FlatNodeId, fresh_stmt_id ast.FlatNodeId, final_stmt_id ast.FlatNodeId, param_names map[string]bool, param_ids_by_name map[string]ast.FlatNodeId) ?AutofreeMoveProofFact {
	fresh := e.collect_autofree_fresh_local_from_stmt(flat, fn_key, fn_name, fresh_stmt_id,
		param_names) or { return none }
	if !autofree_collect_fresh_local_is_empty_dynamic_array(fresh) {
		return none
	...
	if !e.autofree_collect_prefixed_final_clone_prefix_is_safe(flat, prefix_stmt_id, fresh.name,
		param_names) {
		return none
	}
	return e.collect_autofree_fresh_array_final_clone_move_proof_from_fresh(flat, fresh,
...
fn (mut e Environment) autofree_collect_prefixed_final_clone_prefix_is_safe(flat &ast.FlatAst, stmt_id ast.FlatNodeId, target_name string, param_names map[string]bool) bool {
	if !autofree_collect_node_is(flat, stmt_id, .stmt_assign)
	...
	if !autofree_collect_node_is(flat, lhs_id, .expr_ident)
		|| autofree_collect_exact_edge_count(flat, lhs_id) != 0
		|| !autofree_collect_node_is(flat, rhs_id, .expr_basic_literal)
		|| autofree_collect_exact_edge_count(flat, rhs_id) != 0 {
		return false
	}
	lhs_name := flat.string_at(flat.nodes[lhs_id].name_id)
	...
	lhs_typ := e.get_expr_type(flat.nodes[lhs_id].pos.id) or { return false }
	rhs_typ := e.get_expr_type(flat.nodes[rhs_id].pos.id) or { return false }
	return e.autofree_collect_prefixed_final_clone_prefix_type_is_safe(lhs_typ)
		&& e.autofree_collect_prefixed_final_clone_prefix_type_is_safe(rhs_typ)
}

fn (mut e Environment) autofree_collect_prefixed_final_clone_prefix_type_is_safe(typ Type) bool {
	if !type_has_valid_payload(typ) {
		return false
	}
	shape := e.autofree_resource_shape(typ)
...
fn (mut e Environment) collect_autofree_fresh_array_final_clone_move_proof_from_fresh(flat &ast.FlatAst, fresh AutofreeFreshLocalFact, final_stmt_id ast.FlatNodeId, param_names map[string]bool, param_ids_by_name map[string]ast.FlatNodeId) ?AutofreeMoveProofFact {
...
fn autofree_collect_natural_release_candidate_from_move_proof(proof AutofreeMoveProofFact) ?AutofreeNaturalReleaseCandidateFact {
	...
	expected_resource := autofree_collect_natural_release_resource_for_kind(proof.kind)
	if expected_resource == .unknown || proof.resource != expected_resource
		|| target.resource != expected_resource || source.resource != expected_resource {
		return none
	}
	shape := proof.shape
...
fn autofree_collect_natural_release_resource_for_kind(kind AutofreeMoveProofKind) AutofreeResourceKind {
...
fn autofree_collect_release_plan_action_for_kind(kind AutofreeMoveProofKind) AutofreeReleasePlanAction {
	if kind == .fresh_local_binding || kind == .local_array_clone_binding
		|| kind == .receiver_field_slice_clone_binding || kind == .loop_local_clone_push_binding {
		return .array_container_cleanup
	}
	if kind == .fresh_local_string_clone_push_binding
		|| kind == .fresh_local_string_borrowed_call_binding {
		return .string_value_cleanup
	}
	return .unknown
}
...
fn autofree_collect_source_endpoint_matches_natural_release_kind(kind AutofreeMoveProofKind, source AutofreeTransferEndpoint) bool {
	match kind {
		...
		.fresh_local_string_clone_push_binding {
			return autofree_collect_fresh_string_clone_push_source_endpoint_is_allowed(source)
		}
		.fresh_local_string_borrowed_call_binding {
			return autofree_collect_fresh_string_borrowed_call_source_endpoint_is_allowed(source)
		}
		else {
			return false
		}
...
fn autofree_collect_natural_release_shape_allowed(kind AutofreeMoveProofKind, shape AutofreeResourceShape, source AutofreeTransferEndpoint, typ Type) bool {
	match kind {
		...
		.fresh_local_string_clone_push_binding {
			return autofree_collect_fresh_string_cleanup_shape_allowed(shape)
				&& source.reason == 'owned string plus call result'
		}
		.fresh_local_string_borrowed_call_binding {
			return autofree_collect_fresh_string_cleanup_shape_allowed(shape)
				&& autofree_collect_fresh_string_borrowed_call_source_endpoint_is_allowed(source)
		}
		else {
			return false
		}
...
fn autofree_collect_fresh_string_clone_push_source_endpoint_is_allowed(source AutofreeTransferEndpoint) bool {
...
fn autofree_collect_fresh_string_borrowed_call_source_endpoint_is_allowed(source AutofreeTransferEndpoint) bool {
	if source.storage != .call_result || source.root_storage != .call_result || source.path.len != 0
		|| source.root_node_id != source.node_id || source.root_pos_id != source.pos_id
		|| source.state != .owned_unique || source.root_name != source.name {
		return false
	}
	if source.root_name == 'string__plus' && source.reason == 'owned string plus call result' {
		return true
	}
	return source.root_name in ['int__str', 'i64__str', 'u64__str']
		&& source.reason == 'owned string str call result'
}
...
fn autofree_collect_release_plan_from_candidate(candidate AutofreeNaturalReleaseCandidateFact, proof AutofreeMoveProofFact) ?AutofreeReleasePlanFact {
	...
	expected_resource := autofree_collect_natural_release_resource_for_kind(candidate.move_kind)
	if expected_resource == .unknown || candidate.resource != expected_resource
		|| proof.resource != expected_resource || source.resource != expected_resource
		|| target.resource != expected_resource {
		return none
	}
	shape := candidate.shape
	...
	plan_action := autofree_collect_release_plan_action_for_kind(candidate.move_kind)
	if plan_action == .unknown {
		return none
	}
...
fn autofree_collect_release_preflight_from_plan(plan AutofreeReleasePlanFact) ?AutofreeReleasePreflightFact {
	...
	plan_action := autofree_collect_release_plan_action_for_kind(plan.move_kind)
	if plan_action == .unknown {
		return none
	}
	if plan.plan_kind != .natural_exit || plan.plan_action != plan_action
	...
	expected_resource := autofree_collect_natural_release_resource_for_kind(plan.move_kind)
	if expected_resource == .unknown || plan.resource != expected_resource
...
fn autofree_collect_release_insertion_point_from_preflight(preflight AutofreeReleasePreflightFact) ?AutofreeReleaseInsertionPointFact {
	...
	plan_action := autofree_collect_release_plan_action_for_kind(preflight.move_kind)
	if plan_action == .unknown {
		return none
	}
	if !autofree_collect_natural_release_move_kind_is_supported(preflight.move_kind)
		|| preflight.plan_kind != .natural_exit || preflight.plan_action != plan_action
	...
	expected_resource := autofree_collect_natural_release_resource_for_kind(preflight.move_kind)
	if expected_resource == .unknown || preflight.resource != expected_resource
		|| source.resource != expected_resource || target.resource != expected_resource {
		return none
	}
...
fn (mut e Environment) collect_autofree_map_value_store_transfer_from_stmt(flat &ast.FlatAst, fn_key string, fn_name string, stmt_id ast.FlatNodeId, fresh AutofreeFreshLocalFact, param_names map[string]bool, param_bindings map[string]AutofreeBindingFact) ?AutofreeTransferFact {
	...
	root_map := autofree_collect_direct_map_payload(root_typ) or { return none }
	map_key_typ := root_map.key_type
	map_value_typ := root_map.value_type
...
fn (mut e Environment) collect_autofree_array_element_store_transfer_from_stmt(flat &ast.FlatAst, fn_key string, fn_name string, stmt_id ast.FlatNodeId, fresh AutofreeFreshLocalFact, param_names map[string]bool, param_bindings map[string]AutofreeBindingFact) ?AutofreeTransferFact {
	...
	root_array := autofree_collect_direct_array_payload(root_typ) or { return none }
	array_elem_typ := root_array.elem_type
...
fn (mut e Environment) collect_autofree_field_array_push_transfer_from_stmt(flat &ast.FlatAst, fn_key string, fn_name string, stmt_id ast.FlatNodeId, fresh AutofreeFreshLocalFact, param_names map[string]bool, param_bindings map[string]AutofreeBindingFact) ?AutofreeTransferFact {
	...
	lhs_array := autofree_collect_direct_array_payload(lhs_typ) or { return none }
	array_elem_typ := lhs_array.elem_type
...
fn (mut e Environment) collect_autofree_prior_local_array_push_transfer_from_stmt(flat &ast.FlatAst, fn_key string, fn_name string, stmt_id ast.FlatNodeId, alias AutofreePriorLocalAliasProof, param_bindings map[string]AutofreeBindingFact) ?AutofreeTransferFact {
	...
	root_binding_array := autofree_collect_direct_array_payload(root_binding_typ) or { return none }
	root_binding_elem_typ := root_binding_array.elem_type
...
fn (mut e Environment) collect_autofree_return_struct_init_borrowed_pointer_field_transfer_from_stmt(flat &ast.FlatAst, fn_key string, fn_name string, stmt_id ast.FlatNodeId, return_type_id ast.FlatNodeId, fn_scope &Scope, param_names map[string]bool, param_bindings map[string]AutofreeBindingFact) ?AutofreeTransferFact {
	...
	return_struct := autofree_collect_direct_struct_payload(return_typ) or { return none }
...
fn (mut e Environment) collect_autofree_struct_init_field_sumtype_wrap_payload_transfer_from_stmt(flat &ast.FlatAst, fn_key string, fn_name string, stmt_id ast.FlatNodeId, fn_scope &Scope, param_names map[string]bool, param_bindings map[string]AutofreeBindingFact) ?AutofreeTransferFact {
	...
	lhs_struct := autofree_collect_direct_struct_payload(lhs_typ) or { return none }
...
fn (mut e Environment) collect_autofree_local_holder_return_interop_transfer_from_stmt(flat &ast.FlatAst, fn_key string, fn_name string, stmt_id ast.FlatNodeId, return_type_id ast.FlatNodeId, wrap_transfer AutofreeTransferFact, fn_scope &Scope) ?AutofreeTransferFact {
	...
	holder_struct := autofree_collect_direct_struct_payload(holder_typ) or { return none }

	holder_scope_struct := autofree_collect_direct_struct_payload(holder_scope_typ) or {
		return none
	}

	return_use_struct := autofree_collect_direct_struct_payload(return_use_typ) or { return none }
...
fn (mut e Environment) collect_autofree_for_in_field_array_push_transfer_from_stmt(flat &ast.FlatAst, fn_key string, fn_name string, stmt_id ast.FlatNodeId, fn_scope &Scope, param_bindings map[string]AutofreeBindingFact) ?AutofreeTransferFact {
	...
	iterable_array := autofree_collect_direct_array_payload(iterable_typ) or { return none }
	iterable_elem_typ := iterable_array.elem_type

	iterable_scope_array := autofree_collect_direct_array_payload(iterable_scope_typ) or {
		return none
	}
	iterable_scope_elem_typ := iterable_scope_array.elem_type

	iterable_binding_array := autofree_collect_direct_array_payload(iterable_binding_typ) or {
		return none
	}
	iterable_binding_elem_typ := iterable_binding_array.elem_type

	lhs_array := autofree_collect_direct_array_payload(lhs_typ) or { return none }
	dest_elem_typ := lhs_array.elem_type
...
fn (mut e Environment) collect_autofree_for_in_array_push_transfer_from_stmt(flat &ast.FlatAst, fn_key string, fn_name string, stmt_id ast.FlatNodeId, fn_scope &Scope, param_bindings map[string]AutofreeBindingFact) ?AutofreeTransferFact {
	...
	root_array := autofree_collect_direct_array_payload(root_typ) or { return none }
	root_elem_typ := root_array.elem_type

	root_scope_array := autofree_collect_direct_array_payload(root_scope_typ) or { return none }
	root_scope_elem_typ := root_scope_array.elem_type
...
fn autofree_collect_array_push_schema_accepts_literal_rhs(flat &ast.FlatAst, stmt_id ast.FlatNodeId) bool {
	if !autofree_collect_node_is(flat, stmt_id, .stmt_expr)
		|| autofree_collect_exact_edge_count(flat, stmt_id) != 1 {
		return false
	}
...
fn autofree_collect_lowered_array_push_literal_root_and_rhs_ids(flat &ast.FlatAst, stmt_id ast.FlatNodeId) ?(ast.FlatNodeId, ast.FlatNodeId) {
	if !autofree_collect_node_is(flat, stmt_id, .stmt_expr)
		|| autofree_collect_exact_edge_count(flat, stmt_id) != 1 {
		return none
	}
	...
	callee_id := flat.edges[flat.nodes[push_id].first_edge].child_id
	if !autofree_collect_node_is(flat, callee_id, .expr_ident)
		|| autofree_collect_exact_edge_count(flat, callee_id) != 0
		|| flat.string_at(flat.nodes[callee_id].name_id) != 'builtin__array_push_noscan' {
		return none
	}
	arr_ptr_id := flat.edges[flat.nodes[push_id].first_edge + 1].child_id
	root_id := autofree_collect_lowered_array_push_target_root_id(flat, arr_ptr_id) or {
		return none
	}
	wrapper_id := flat.edges[flat.nodes[push_id].first_edge + 2].child_id
	if !autofree_collect_node_is(flat, wrapper_id, .expr_array_init)
		|| autofree_collect_exact_edge_count(flat, wrapper_id) != 6 {
		return none
	}
	rhs_id := flat.edges[flat.nodes[wrapper_id].first_edge + 5].child_id
	if !autofree_collect_node_is(flat, rhs_id, .expr_basic_literal)
		|| autofree_collect_exact_edge_count(flat, rhs_id) != 0 {
		return none
	}
	return root_id, rhs_id
...
fn autofree_collect_lowered_array_push_target_root_id(flat &ast.FlatAst, arr_ptr_id ast.FlatNodeId) ?ast.FlatNodeId {
	if !autofree_collect_node_is(flat, arr_ptr_id, .expr_cast)
		|| autofree_collect_exact_edge_count(flat, arr_ptr_id) != 2 {
		return none
	}
	typ_id := flat.edges[flat.nodes[arr_ptr_id].first_edge].child_id
	if !autofree_collect_node_is(flat, typ_id, .expr_ident)
		|| autofree_collect_exact_edge_count(flat, typ_id) != 0
		|| flat.string_at(flat.nodes[typ_id].name_id) != 'array*' {
		return none
	}
	amp_id := flat.edges[flat.nodes[arr_ptr_id].first_edge + 1].child_id
	if !autofree_collect_node_is(flat, amp_id, .expr_prefix)
		|| autofree_collect_exact_edge_count(flat, amp_id) != 1
		|| unsafe { token.Token(int(flat.nodes[amp_id].aux)) } != .amp {
		return none
	}
	root_id := flat.edges[flat.nodes[amp_id].first_edge].child_id
	if !autofree_collect_node_is(flat, root_id, .expr_ident)
		|| autofree_collect_exact_edge_count(flat, root_id) != 0 {
		return none
	}
...
fn autofree_collect_direct_struct_field_type(lhs_typ Type, init_typ Type, field_name string) ?Type {
	...
	init_struct := autofree_collect_direct_struct_payload(init_typ) or { return none }
...
fn autofree_collect_direct_struct_scope_field_type(lhs_typ Type, init_typ Type, lhs_scope_typ Type, field_name string) ?Type {
	...
	scope_struct := autofree_collect_direct_struct_payload(lhs_scope_typ) or { return none }
...
fn autofree_collect_same_direct_struct_field_type(left Type, right Type) bool {
	...
	return match left {
		Struct {
			left_struct := autofree_collect_direct_struct_payload(left) or { return false }
			right_struct := autofree_collect_direct_struct_payload(right) or { return false }
			autofree_collect_same_direct_struct_payload(left_struct, right_struct)
		}
		SumType {
			left_sumtype := autofree_collect_direct_sumtype_payload(left) or { return false }
			right_sumtype := autofree_collect_direct_sumtype_payload(right) or { return false }
			autofree_collect_same_direct_sumtype_payload(left_sumtype, right_sumtype)
		}
		else {
...
fn autofree_collect_direct_struct_payload(typ Type) ?Struct {
	if !type_has_valid_payload(typ) || typ !is Struct {
		return none
	}
	return typ as Struct
}

fn autofree_collect_direct_map_payload(typ Type) ?Map {
	if !type_has_valid_payload(typ) || typ !is Map {
		return none
	}
	return typ as Map
}

fn autofree_collect_direct_array_payload(typ Type) ?Array {
	if !type_has_valid_payload(typ) || typ !is Array {
		return none
	}
	return typ as Array
}
...
fn autofree_collect_direct_sumtype_wrap_target(lhs_typ Type, wrap_typ Type, lhs_scope_typ Type) ?SumType {
	...
	lhs_sumtype := autofree_collect_direct_sumtype_payload(lhs_typ) or { return none }

	wrap_sumtype := autofree_collect_direct_sumtype_payload(wrap_typ) or { return none }

	scope_sumtype := autofree_collect_direct_sumtype_payload(lhs_scope_typ) or { return none }
...
fn autofree_collect_direct_sumtype_payload(typ Type) ?SumType {
	if !type_has_valid_payload(typ) || typ !is SumType {
		return none
	}
	return typ as SumType
}
